...
* Fixed inconsistent data which could happen when the synchro queue was full
  ...
  manually via `fiber:wakeup()` or `fiber:cancel()`). Specifically, txns could
  get committed in a wrong order or newly joined replicas could have data not
  present on the master (gh-11180).
