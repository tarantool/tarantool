...
struct random_generator {
	...
	uint64_t get()
	{
		...
		{
		case 0:
			return std::uniform_int_distribution<uint8_t>(0, UINT8_MAX)(
				generator);
		case 1:
			return std::uniform_int_distribution<uint16_t>(0, UINT16_MAX)(
				generator);
		case 2:
			return std::uniform_int_distribution<uint32_t>(0, UINT32_MAX)(
				generator);
		case 3:
			return std::uniform_int_distribution<uint64_t>(0, UINT64_MAX)(
				generator);
		default:
			unreachable();
			return 0;
...
public:
	static struct tuple_format *make(struct key_def *kd)
	{
		...
		struct tuple_format *fmt = tuple_format_new(
			&memtx_tuple_format_vtab, memtx, &kd, 1, fields, FIELD_COUNT,
			FIELD_COUNT, dict, false, false, NULL, 0, NULL, 0);
		if (fmt == NULL)
			abort();
		tuple_format_ref(fmt);
...
public:
	...
	MpData()
	{
		...
		for (size_t i = 0; i < field_count; i++) {
			if (is_non_null[i])
				data_end = mp_encode_uint(data_end, random_gen.get());
			else
				data_end = mp_encode_nil(data_end);
		}
		if (data_end - data > MAX_TUPLE_DATA_SIZE)
