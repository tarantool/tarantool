...
def create_csv_report(client, bucket, branch, bench_name, subtest_name, hours, csv_filename):
    ...
    for row in csv_iterator:
        ...
        row[time_idx] = parser.parse(row[time_idx]).strftime("%Y-%m-%d %H:%M:%S %z")
        writer.writerow(row)
    csv_file.close()
    ...
    with open(args.config) as f:
        ...
        for test_config in bench_configs:
            ...
            for subtest_name in test_config["subtests"]:
                ...
                csv_path = os.path.join(args.base_path, csv_filename)
                print("Build report {}: {}, {}".format(bench_name, subtest_name, csv_path))
                create_csv_report(client, args.influx_bucket, args.branch_name, bench_name,
                    subtest_name, args.hours, csv_path)

                # Build a test config for Apache Otava configuration.
                otava_test_config = copy.deepcopy(default_test_config)
                otava_test_config["file"] = os.path.join(args.base_path, csv_filename)
