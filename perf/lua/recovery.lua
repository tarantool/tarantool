...
local HELP = [[
   ...
   wal_row_count <number, 0>    - the amount of tuples in the WAL
...
local WORK_DIR = string.format('recovery,engine=%s,index=%s,nohint=%s,' ..
                               'sk_count=%d,row_count=%d,wal_row_count=%d,' ..
                               'column_count=%d', params.engine, params.index,
                               params.nohint, params.sk_count, params.row_count,
                               params.wal_row_count, params.column_count)
fio.mkdir(WORK_DIR)
...
box.once('init', function()
    ...
    for i = 1, sk_count do
        s:create_index('sk' .. i, {type = index, hint = not nohint,
                                   unique = false})
    end
    local tuple = {0}
    for i = 2, column_count do
        tuple[i] = math.random(1, 1000000)
    end
