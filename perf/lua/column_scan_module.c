...
static void
arrow_array_fill_validity(struct ArrowArray *array, int count, int fill_ratio)
{
	bool bits[count];
	memset(bits, 0, sizeof(bits));
	for (int i = 0; i < count * fill_ratio / 100; i++)
		bits[i] = true;
...
arrow_array_fill_str(struct ArrowArray *array, int column, int row_count,
		     int row_offset, int len_min, int len_max, int fill_ratio)
{
...
arrow_array_fill_int(struct ArrowArray *array, int column_count, int row_count,
		     int row_offset, int total_row_count, int fill_ratio)
{
	for (int i = 0; i < column_count; i++) {
		if (i > 0)
...
gen_arrow_lua_func(struct lua_State *L)
{
	...
	int fill_ratio = luaL_checkinteger(L, 7);
