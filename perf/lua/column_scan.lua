...
local USAGE = [[
   ...
   fill_ratio <number, 100>         - number of non-NULL values / number of NULL
                                      values (in percent, [0..100])
   layout <string, ''>              - column layout option of the index (empty
                                      string if not supported by the engine)
...
params.fill_ratio = params.fill_ratio or 100
params.layout = params.layout or ''
...
if params.fill_ratio > 100 then
    die("Fill ratio must be in the range from 0 to 100")
end
...
local function gen_insert(space_id, column_count, row_count, field_type,
    ...
    for i = 1, row_count do
        if fill_ratio < math.random(100) then
            tuple[1] = i
...
local TESTS = {
    ...
    },
    {
        ...
        type = 'unsigned',
        func = function()
            return test_funcs.sum(box.space.test.id, 0, params.column_count - 1)
        end,
    },
    {
        name = 'string_notouch',
