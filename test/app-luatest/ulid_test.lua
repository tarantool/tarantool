...
g.test_monotonic_and_unique = function()
    ...
    for _ = 2, n do
        local cur = ulid.str()
        t.assert(prev <= cur, ('ULID order break: %s > %s'):format(prev, cur))
        t.assert(not seen[cur], ('duplicate ULID at: %s'):format(cur))
        seen[cur] = true
        prev = cur
    end
end
...
g.test_eq_and_nil = function()
    ...
    t.assert_equals(ulid.NULL, ulid.fromstr(null_str))
    t.assert(ulid.NULL:isnil())
    t.assert_not_equals(ulid.NULL:isnil())
