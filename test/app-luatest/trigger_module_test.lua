...
g.before_all(function()
    ...
    rawset(_G, 'gen_callable_objects', generate_callable_objects)
    end)
end)
...
g.test_exceptions = function()
    g.server:exec(function()
        ...
        t.assert_error_msg_content_equals(lua_err, trigger.call, event,
            'throw lua')
        t.assert_equals(state, 0)
        t.assert_error_msg_content_equals(tnt_err, trigger.call, event,
            'throw tnt')
        t.assert_equals(state, 0)
...
g.test_any_callable_trigger = function()
    g.server:exec(function()
        local trigger = require('trigger')
        local generate_callable_objects = rawget(_G, 'gen_callable_objects')
        ...
        -- Callable cdata
        check_object(c)
        -- Cdata metatable is already protected so a check is not required
    end)
end
...
g.test_trigger_clear_from_trigger = function()
    g.server:exec(function()
        ...
        trigger.call(event)
        t.assert_equals(order, {1}, "Correct trigger invocation when 1st \
                                    trigger clears 2nd")
    end)
end
...
g.test_invalid_args = function()
    g.server:exec(function()
        ...
        local invalid_handlers = {nil, 42, 'abc', {}, {1, 2}, {k = 'v'},
            box.NULL}
        ...
        for _, invalid_name in pairs(invalid_names) do
            ...
            t.assert_error_msg_content_equals(errmsg, trigger.pairs,
                invalid_name)
            t.assert_error_msg_content_equals(errmsg, trigger.info,
                invalid_name)
        end
        local errmsg = "Usage: trigger.set(event, trigger, function)"
        t.assert_error_msg_content_equals(errmsg, trigger.set, 'event',
            'trigger', handler, 'redundant')
        t.assert_error_msg_content_equals(errmsg, trigger.set, 'event',
            'trigger')
