...
g_cfg.test_invalid_options = function()
    local s = json.new()
    s.cfg({decode_overflow = 'string', encode_key_order = {'a', 'b'}})
    local max_depth = s.cfg.decode_max_depth
    for _, value in ipairs({
        '', 'unknown', 'CLAMP', 'clamp\0error', 'error\0', 0, 1, false, true,
        {}, function() end, json.NULL,
    }) do
        local options = {
            decode_overflow = value,
            decode_max_depth = 1,
            encode_invalid_numbers = false,
            encode_key_order = {'b', 'a'},
        }
        ...
        t.assert_equals(s.cfg.decode_max_depth, max_depth)
        t.assert_equals(s.cfg.encode_invalid_numbers, true)
        t.assert_equals(s.cfg.encode_key_order, {'a', 'b'})
        t.assert_equals(s.decode(positive), positive)
        t.assert_equals(s.decode('{"a":{"b":1}}'), {a = {b = 1}})
        t.assert_equals(s.encode({a = 1, b = 2}), '{"a":1,"b":2}')
        t.assert_equals(s.encode(math.huge), 'inf')
    end
    t.assert_equals(s.decode(positive, {decode_overflow = nil}), positive)
end

g_cfg.before_test('test_invalid_msgpack_cfg', function(cg)
    cg.encode_invalid_numbers = msgpack.cfg.encode_invalid_numbers
    msgpack.cfg({encode_invalid_numbers = true})
end)

g_cfg.after_test('test_invalid_msgpack_cfg', function(cg)
    msgpack.cfg({encode_invalid_numbers = cg.encode_invalid_numbers})
end)

g_cfg.test_invalid_msgpack_cfg = function()
    t.assert_error_msg_contains('Invalid decode_overflow', msgpack.cfg, {
        encode_invalid_numbers = false,
        decode_overflow = 'typo',
    })
    -- Rejected options must not desynchronize the dependent tuple serializer.
    local msgpack_ok = pcall(msgpack.encode, {math.huge})
    local tuple_ok = pcall(box.tuple.new, {math.huge})
    t.assert_equals(msgpack_ok, tuple_ok)
    t.assert(msgpack_ok)
    t.assert_equals(msgpack.cfg.encode_invalid_numbers, true)

    msgpack.cfg({encode_invalid_numbers = false})
    t.assert_error_msg_contains('number must not be NaN or Inf',
                               msgpack.encode, {math.huge})
    t.assert_error_msg_contains('number must not be NaN or Inf',
                               box.tuple.new, {math.huge})
end
...
local g_locale = t.group('json_integer_overflow_locale', {
    {api = 'cfg'}, {api = 'decode'},
})

g_locale.before_each(function(cg)
    cg.numeric_locale = os.setlocale(nil, 'numeric')
    for _, locale in ipairs({'de_DE.UTF-8', 'de_DE.utf8', 'ru_RU.UTF-8',
                             'fr_FR.UTF-8'}) do
        if os.setlocale(locale, 'numeric') ~= nil then
            return
        end
    end
    t.skip('A locale with a decimal comma is required')
end)

g_locale.after_each(function(cg)
    os.setlocale(cg.numeric_locale, 'numeric')
end)

g_locale.test_number_token_boundary = function(cg)
    local s = json.new()
    local options = {decode_overflow = 'number', decode_invalid_numbers = false}
    if cg.params.api == 'cfg' then
        s.cfg(options)
    ...
    for _, case in ipairs({{positive, 2^64}, {negative, -2^63}}) do
        local value, number = unpack(case)
        t.assert_equals(s.decode('[' .. value .. ',1,2]', options),
                        {number, 1, 2})
        t.assert_equals(s.decode('[' .. value .. ',1e300]', options),
                        {number, 1e300})
        t.assert_equals(s.decode('["before",' .. value .. ',"after"]', options),
                        {'before', number, 'after'})
        t.assert_equals(s.decode('{"a":' .. value .. ',"b":1}', options),
                        {a = number, b = 1})
        t.assert_error(s.decode, value .. ',1', options)
    end
    t.assert_error_msg_contains('number must not be NaN or Inf', s.decode,
                               string.rep('9', 400), options)
end
