...
--
-- NB: luatest.server is not used, because it writes to the _priv system space
-- to grant a test user a permission to execute code (so :exec() works). Use
-- justrun instead.
...
g.test_index = function(cg)
    ...
    -- Track find_entries() calls to do self-check (see the end of the test
    -- case).
    ...
    --
    -- NB: The indexes declaration is the same as for _space (except space
    -- id). This is not a property we must retain, more like a coincidence.
    -- We use this fact in the test just to save typing.
    local expected_sequence_entries = expected_space_entries
    ...
    -- id). This is not a property we must retain, more like a coincidence.
    -- We use this fact in the test just to save typing.
    local expected_func_entries = expected_space_entries
    ...
    -- id). This is not a property we must retain, more like a coincidence.
    -- We use this fact in the test just to save typing.
    local expected_user_entries = expected_space_entries
