...
g.before_all(function(cg)
    ...
    cg.server:exec(function()
        ...
        rawset(_G, 'slow_fn', function(seconds)
            require('fiber').sleep(seconds)
            return 'done'
        end)
        ...
        box.schema.user.grant('guest', 'execute', 'function', 'slow_fn',
                              {if_not_exists = true})
        rawset(_G, 'sleep_fn', function(seconds)
            require('fiber').sleep(seconds)
            return 'done'
        end)
        box.schema.func.create('sleep_fn',
            {language = 'LUA', if_not_exists = true})
        box.schema.user.grant('guest', 'execute', 'function', 'sleep_fn',
                              {if_not_exists = true})
    end)
end)
...
g.test_no_kill_during_request = function(cg)
    ...
    t.assert_equals(conn.state, 'active')
    conn:ping()
    local fut = conn:call('slow_fn', {TIMEOUT * 5}, {is_async = true})
    fiber.sleep(TIMEOUT * 3)
    t.assert_equals(conn.state, 'active')
...
g.test_invalid_config_dynamic = function(cg)
    cg.server:exec(function()
        ...
        t.assert_error_msg_content_equals(
            prefix .. 'user name must be a string', set, 5, 5)
        t.assert_error_msg_content_equals(
            prefix .. 'timeout must be a number', set, 'guest', 'x')
        t.assert_error_msg_content_equals(
            prefix .. 'timeout must be >= 0', set, 'guest', -1)
    end)
...
g.test_timer_starts_when_request_ends = function(cg)
    ...
    local conn = net.connect(cg.server.net_box_uri,
                             {user = 'bob', password = 'secret'})
    t.assert_equals(conn:call('sleep_fn', {TIMEOUT * 2}), 'done')
