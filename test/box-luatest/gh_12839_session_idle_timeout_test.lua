...
g.test_invalid_config_dynamic = function(cg)
    cg.server:exec(function()
        ...
        t.assert_error_covers({
            type = "ClientError",
            name = "CFG",
            details = "user name must be a string",
...
g.before_test('test_throttling', function(cg)
    -- Readahed throttling required requests in flight, such
    -- connections cannot be killed by idle timeout.
    cg.net_msg_max = cg.server:exec(function(timeout)
        box.internal.session.idle_timeout_set('bob', timeout)
        local old = box.cfg.net_msg_max
        box.cfg{net_msg_max = 2}
        return old
    end, {TIMEOUT})
end)

g.after_test('test_throttling', function(cg)
    cg.server:exec(function(net_msg_max)
        box.cfg{net_msg_max = net_msg_max}
    end, {cg.net_msg_max})
    cg.net_msg_max = nil
end)

g.test_throttling = function(cg)
    local conn_a = net.connect(cg.server.net_box_uri,
                               {user = 'bob', password = 'secret'})
    local conn_b = net.connect(cg.server.net_box_uri,
                               {user = 'bob', password = 'secret'})
    t.assert_equals(conn_a.state, 'active')
    t.assert_equals(conn_b.state, 'active')
    conn_b:ping()

    local futs_a = {}
    for i = 1, 3 do
        futs_a[i] = conn_a:eval([[
            local fiber = require('fiber')
            fiber.sleep(...)
            return 'done'
        ]], {TIMEOUT * 5}, {is_async = true})
    ...
    local fut_b = conn_b:eval('return true', {}, {is_async = true})

    fiber.sleep(TIMEOUT * 3)
    t.assert_equals(conn_b.state, 'active')
    -- The request is still pending: the input was indeed throttled.
    t.assert_not(fut_b:is_ready())

    t.assert_equals(fut_b:wait_result(10)[1], true)
    local drained_at = fiber.clock()
    assert_closed_by_idle(conn_b)
    t.assert_ge(fiber.clock() - drained_at, TIMEOUT / 2)

    for _, fut in ipairs(futs_a) do
        t.assert_equals(fut:wait_result(10)[1], 'done')
    end
    conn_a:close()
    conn_b:close()
end
