...
g.test_net_replicaset_alerts = function()
    ...
    local function wait_alert(rs, needle)
        t.helpers.retrying({timeout = 60}, function()
            local alerts = rs:info().alerts
            t.assert_equals(#alerts, 1)
            t.assert_str_contains(alerts[1].message, needle)
        end)
    end
    ...
    -- The whole replicaset is unreachable: the only instance never connects.
    local rs = connect({bogus = unreachable})
    wait_alert(rs, 'is unreachable')
    rs:close()
    ...
    wait_alert(rs, 'has no writable leader')
    rs:close()
