...
g.test_session_triggers = function(cg)
    cg.server:exec(function()
        ...
        local res = conn:eval([[
return box.space["tweedledum"]:get{box.session.id()}[1] == box.session.id()]])
...
g.test_session_gh_2994 = function(cg)
    ...
    t.assert_not_equals(
        conn:eval('return box.session.peer(box.session.id())'),
...
g.test_session_gh_2994 = function(cg)
    ...
    local net_box = require('net.box')
    local conn = net_box.connect(uri)
    -- t.assert_equals(box.session.euid(), 1, 'session.uid')
    -- t.assert_equals(box.session.su('guest', box.session.uid), 1,
    --                 'session.uid from su is admin')
    -- t.assert_equals(box.session.su('guest', box.session.euid), 0,
    --                 'session.euid from su is guest')
