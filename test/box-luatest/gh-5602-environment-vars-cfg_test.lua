...
local listen_sock1 = fio.pathjoin(tmpdir, 'listen1.sock')
local listen_sock2 = fio.pathjoin(tmpdir, 'listen2.sock')
...
os.exit(test:check() and 0 or 1, true) ]]):format(listen_sock1)
    treegen.write_file(dir, script_name, script_body)
    local opts = { nojson = true,  stderr = true }
    local env = {
        TT_LISTEN = listen_sock1,
        TT_READAHEAD = 10000,
        TT_STRIP_CORE = false,
        TT_LOG_FORMAT = 'json',
