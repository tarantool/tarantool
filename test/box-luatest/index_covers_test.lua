...
g.test_covers_validation = function(cg)
    cg.server:exec(function()
        ...
        t.assert_error_covers({
            type = 'IllegalParams',
            message = "options.covers[1]: field is larger than number of" ..
                      " fields",
        }, s.create_index, s, 'sk', {covers = {3}})
        t.assert_error_covers({
            type = 'IllegalParams',
        ...
        t.assert_error_covers({
            type = 'IllegalParams',
            message = "options.covers[2]: duplicate field",
        ...
        t.assert_error_covers({
            type = 'ClientError',
            code = box.error.WRONG_INDEX_OPTIONS,
            message = "Wrong index options: 'covers' field is larger that" ..
                      " number of fields",
        }, box.space._index.insert, box.space._index, {
            s.id, 1, 'sk', 'tree', {unique = true, covers = {2}},
            {{0, 'unsigned'}}
        })
        t.assert_error_covers({
            type = 'ClientError',
...
g.test_covers_stubs = function(cg)
    cg.server:exec(function()
        ...
        t.assert_error_covers({
            ...
            message = "memtx does not support covers option",
        }, s.create_index, s, 'sk', {covers = {1}})
        s:drop()
        local s = box.schema.create_space('test', {
            engine = 'vinyl',
        ...
        t.assert_error_covers({
            type = 'ClientError',
            code = box.error.UNSUPPORTED,
            message = "vinyl does not support covers option",
        }, s.create_index, s, 'sk', {covers = {1}})
    end)
end
