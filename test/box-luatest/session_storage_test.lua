...
g.test_session_storage = function(cg)
    ...
    t.assert_equals(is_equal, true, 'check meta table')
    ...
    t.assert_equals(abc_is_cde, true, 'first conn storage')
    local abc_is_cde_conn1 =
        conn1:eval('return all[box.session.id()].abc == "cde"')
    t.assert_equals(abc_is_cde_conn1, true, 'check first conn metatable')
    local abc_is_def_conn2 =
        conn2:eval('return all[box.session.id()].abc == "def"')
    t.assert_equals(abc_is_def_conn2, true, 'check second conn metatable')
    ...
    t.assert_equals(tres1[1], 'cde', 'check after closing')
    t.assert_equals(#tres2, 0, 'check after closing')
    conn3:close()
end
