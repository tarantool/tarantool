...
local function test_hash_with_num(cg, start_num)
    cg.server:exec(function(start_num)
        ...
        --
        -- Simple hashing, parts = {UINT}.
        --
        local s = make_space_with_parts({{1, 'unsigned'}})
        local key = start_num
        _G.check_int_codec(s, {
            key_num = key, key = {key},
        ...
        --
        -- Simple hashing, parts = {UINT, UINT}.
        --
...
g.test_hash = function(cg)
    ...
    cg.server:exec(function()
        rawset(_G, 'check_int_codec', function(s, opts)
