...
local g = t.group('pagination.read_view.tree',
                  t.helpers.matrix({ffi = {true, false}}))
...
g.test_tree_pagination = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        ...
        box.space.s:replace{0, 0, 0, 0}
        box.space.s:replace{12, 2, 12, 12}
        rv:close()
        rv = box.read_view.open()
        s = rv.space.s
...
g.test_tree_multikey_pagination = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk",
        ...
        -- Fetch position in empty space
        local tuples, pos = s.index.sk:select(nil,
                {limit=2, fullscan=true, fetch_pos=true})
        t.assert_equals(tuples, {})
        t.assert_equals(pos, nil)

        for i = 1, 3 do
            box.space.s:replace{i, {1, 2, 3}}
        end
        rv:close()
        rv = box.read_view.open()
        s = rv.space.s
        ...
        -- Test fullscan pagination
        for i = 0, 4 do
            tuples, pos = s.index.sk:select(nil,
                    {limit=2, fullscan=true, fetch_pos=true, after=pos})
            tuples_offset = s.index.sk:select(nil,
                    {limit=2, fullscan=true, offset=i*2})
        ...
        for iter, key in pairs(key_iter) do
            pos = ""
            for i = 0, 2 do
                tuples, pos = s.index.sk:select(key,
                        {limit=2, iterator=iter, fetch_pos=true, after=pos})
                tuples_offset = s.index.sk:select(key,
                        {limit=2, iterator=iter, offset=i*2})
                t.assert_equals(tuples, tuples_offset)
            end
            tuples, pos = s.index.sk:select(key,
        ...
        for _, iter in pairs({'EQ', 'REQ'}) do
            pos = ""
            for i = 0, 1 do
                tuples, pos = s.index.sk:select(2,
                        {limit=2, iterator=iter, fetch_pos=true, after=pos})
                tuples_offset = s.index.sk:select(2,
                        {limit=2, iterator=iter, offset=i*2})
                t.assert_equals(tuples, tuples_offset)
            end
            tuples, pos = s.index.sk:select(2,
        ...
        t.assert_error_msg_contains(
                ...
                s.index.sk.select, s.index.sk, nil,
                {fullscan=true, after=tuple})
        rv:close()
    end)
end

g.test_nullable_indexes = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk1",
        ...
        for i = 1, 13 do
            if i % 3 == 0 then
                box.space.s:replace{i}
            else
                box.space.s:replace{i, i, i, i, i}
            end
        end
        rv:close()
        rv = box.read_view.open()
...
g.test_no_tuples_satisfying_filters = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk",
        ...
        t.assert_equals(tuples, {})
        t.assert(pos == nil)
        box.space.s:replace{2, {1, 2}}
        box.space.s:replace{3, {1, 2}}
        box.space.s:replace{4, {1, 2}}
        rv:close()
        rv = box.read_view.open()
        s = rv.space.s
...
g.test_invalid_positions = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index('pk', {type = 'tree'})
        s:create_index('sk', {
...
g.test_tuple_pos_simple = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk", {
...
g.test_tuple_pos_corner_cases = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk1", {parts = {{field = 2, type = 'uint'}},
                               type = "tree", unique=true})
        s:create_index("sk2", {parts = {{field = 3, type = 'uint'}},
                               type = "tree", unique=false})
        for i = 0, 9 do
        ...
        local rv = box.read_view.open()
        s = rv.space.s
        local sk1 = s.index.sk1
        local sk2 = s.index.sk2
...
g.test_tuple_pos_invalid_tuple = function(cg)
    cg.server:exec(function()
        local s = box.space.s
...
g.test_pagination_pairs = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:create_index("sk", {parts = {2}, type = "tree", unique=false})
        ...
        indexes = {s.index.pk, s.index.sk2, s.index.sk_unique}
        math.randomseed(os.time())
        for _, index in pairs(indexes) do
            local i = math.random(0, 11)
            local collected_tuples = {{}, {}}
            for _, tuple in index:pairs(nil, {after={i, 1, i, i}}) do
...
g.test_concurrent_pairs = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        ...
        local i = math.random(0, 11)
        local collected_tuples = {{}, {}}
        local pairs_state = {s:pairs(nil, {iterator='ALL', after={i}})}
        local j = math.random(1, 11)
        if j == i then j = i - 1  end
        for _, tuple in s:pairs(nil, {iterator='ALL', after={j}}) do
            table.insert(collected_tuples[2], tuple)
        end
        ...
        expected_tuples[1] = s:select(i, {iterator='GT'})
        expected_tuples[2] = s:select(j, {iterator='GT'})
...
g.test_empty_page_with_offset = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index("pk", {type = "tree"})
        s:replace{1}
...
g.test_after_missing = function(cg)
    cg.server:exec(function()
        local s = box.space.s
...
g.test_invalid_after = function(cg)
    cg.server:exec(function()
...
g.test_func_index_pagination = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        ...
        for i = 1, 6 do
            box.space.s:replace{i, i}
        end
        for i = 7, 10 do
            box.space.s:replace{i, 6}
        end
        box.space.s:replace{11, 7}
        rv:close()
        rv = box.read_view.open()
        s = rv.space.s
        sk = s.index.func
        ...
        rv:close()
        box.space.s.index.func:drop()
        box.schema.func.drop('func')
    end)
end

g.test_func_multikey_index_pagination = function(cg)
    cg.server:exec(function()
        local s = box.space.s
        s:create_index('pk', {parts = {{field = 1, type = 'uint'}}})
...
local no_sup = t.group(
    'pagination.read_view.unsupported',
    t.helpers.matrix({
        ffi = {true, false},
        engine = {'memtx'},
        type = {'hash'},
...
no_sup.test_unsupported_pagination = function(cg)
    cg.server:exec(function(index_type)
        ...
        local pos = s.index.sk:tuple_pos(tuple)
        t.assert_error_msg_contains('does not support pagination',
                s.index.sk.select, s.index.sk,
