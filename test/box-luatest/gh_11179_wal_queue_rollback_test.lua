...
-- batch is returned to TX thread we wakeup fibers in WAL queue. As part of
-- a batch is failed new fibers from queue will fail with cascade rollback
-- error. Unfortunately in this case we do not rollback requests in queue in
-- correct order. Why do we need two batches here? With only one batch we
-- rollback whole queue before requests from queue will pass to WAL.
--
