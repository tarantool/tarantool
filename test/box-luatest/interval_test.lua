...
g.test_interval_insert = function()
    g.server:exec(function()
        ...
        local valbin = '\xc7\x07\x06\x03\x00\x01\x03\x7f\x05\x0f'
        local val = msgpack.decode(valbin)
        ...
        t.assert_equals(msgpack.encode(res), valbin)
        s:drop()
    end)
end
...
g.test_interval_select = function()
    g.server:exec(function()
        ...
        t.assert_equals(msgpack.encode(res1), valbin)
        local res2 = s:select({1})[1][2]
        t.assert_equals(msgpack.encode(res2), valbin)
        s:drop()
    end)
end
