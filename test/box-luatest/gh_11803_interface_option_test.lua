...
g.test_net_box = function()
    ...
    local function check_ok(srv, ifname)
        local conn = net.connect({uri = srv.net_box_uri, interface = ifname},
                                 {wait_connected = true})
    ...
    -- Listen hostname.
    local srv = create_server('localhost:3301')
    check_ok(srv, 'lo')
    check_fail(srv, 'lol', 'suitable interface not found: lol')
    srv:drop()

    -- Listen IPv4.
    srv = create_server('127.0.0.1:3301')
    check_ok(srv, 'lo')
    check_fail(srv, 'lol', 'suitable interface not found: lol')
    srv:drop()
