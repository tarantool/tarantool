...
g.test_lifecycle_and_periodic = function(cg)
    cg.server:exec(function()
        ...
        -- A fresh uuid (hence a fresh label) per attempt.
        ...
        -- Default label format: <instance>.<manager>.<uuid>.
        local instance = type(box.info.name) == 'string' and box.info.name
                         or box.info.uuid
        for _, label in ipairs(created) do
            local prefix, point_uuid = label:match('^(.+)%.m1%.([^.]+)$')
            t.assert_equals(prefix, instance)
            t.assert_not_equals(uuid.fromstr(point_uuid), nil)
        end

        -- A manual create_point() makes one more point with the default label;
        -- an explicit label overrides the default.
        local before = #created
        local res = m:create_point()
        t.assert_equals(res.label, created[#created])
        t.assert_gt(#created, before)
        local labeled = m:create_point({label = 'manual'})
        t.assert_equals(labeled.label, 'manual')
        t.assert_equals(created[#created], 'manual')
...
g.test_available_before_box_cfg = function()
    ...
    local script = [[
        ...
        -- Unconfigured: the label has no instance segment, just
        -- <manager>.<uuid>.
        m:create_point()
