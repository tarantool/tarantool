...
g.test_default_manager = function()
    ...
    local name = info.last_point.name
...
--   * new(cfg) -> an instance with create_point(id, opts) (compulsory) and
--     optional drop()/info().
...
g.test_custom_backend = function()
    ...
    g.cluster.router:exec(function(module_name)
        package.preload[module_name] = function()
            ...
            function instance_methods.create_point(self, id)
                table.insert(self.created, id)
                return {name = id}
            end
