...
g.test_initial_merge = function()
    ...
    treegen.write_file(
        dir, 'included_config.yaml', yaml.encode(included_config))
...
g.test_include_glob = function()
    ...
    treegen.write_file(
        dir, 'conf.d/config_1.yaml', yaml.encode(included_config_1))
    treegen.write_file(
        dir, 'conf.d/config_2.yaml', yaml.encode(included_config_2))
...
g.test_include_multiple = function()
    ...
    treegen.write_file(
        dir, 'included_config_1.yaml', yaml.encode(included_config_1))
    treegen.write_file(
        dir, 'included_config_2.yaml', yaml.encode(included_config_2))
...
g.test_include_recursion = function()
    ...
    g.server:grep_log(
        ...
        '%s/config.yaml"'):format(dir),
        1024, {filename = dir .. '/var/log/instance-001/tarantool.log'}
    )
end
...
g.test_include_erroneous = function()
    ...
    local config_file = treegen.write_file(
        dir, 'config.yaml', yaml.encode(config))
    treegen.write_file(dir, 'errorneous_config.yaml', errorneous_config)
...
g.test_conditional_include = function()
    ...
    -- Remove yaml document start and end markers, because we will need to add
    -- conditional include to the config, which is not a valid yaml.
    local common_config_string = yaml.encode(common_config):sub(5,-6)

    local config = [[
conditional:
- if: tarantool_version >= 0.0.0
  include: [included_config.yaml]
]] .. common_config_string
