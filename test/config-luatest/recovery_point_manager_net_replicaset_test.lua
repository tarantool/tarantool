...
g.test_create_point = function()
    g.cluster.router:exec(function(backend_name)
        ...
        -- create_point returns the created point.
        local point, err = inst:create_point('rpm.test.point', {timeout = 5})
        t.assert_equals(err, nil)
        t.assert_equals(point.name, 'rpm.test.point')
        ...
        -- (nil + err, not a raise).
        inst:drop()
        local point2, err2 = inst:create_point('rpm.test.point2', {timeout = 1})
        t.assert_equals(point2, nil)
        t.assert_not_equals(err2, nil)
    end, {BACKEND})

    -- The point landed on the storage leader.
    g.cluster.storage1:exec(function()
        local row = box.space._recovery_point:get('rpm.test.point')
        t.assert_not_equals(row, nil)
        t.assert_equals(row.name, 'rpm.test.point')
    end)
end
...
g.test_manager = function()
    ...
    -- the `backup` user.
    local point_name = g.cluster.router:exec(function(backend_name)
        ...
        t.assert_type(info.last_point.name, 'string')
        t.assert_not_equals(info.last_point.lsn, nil)

        m:drop()
        t.assert_equals(rp.managers['rpm'], nil)
        return info.last_point.name
    end, {BACKEND})

    -- The manager's last point landed on the storage leader.
    g.cluster.storage1:exec(function(name)
        t.assert_not_equals(box.space._recovery_point:get(name), nil)
    end, {point_name})
end
