...
g.test_discovery_registration = function()
    local ssl_certificate, state = apply_fake_config()
    local calls = {}
    ssl_certificate.register_discovery(function(config, records, add)
...
g.test_discovery_registration_is_idempotent = function()
    local ssl_certificate = apply_fake_config()
    local calls = 0
    local function discover()
        calls = calls + 1
    end
    ssl_certificate.register_discovery(discover)
    ssl_certificate.register_discovery(discover)
    ssl_certificate._discover()
    t.assert_equals(calls, 1)
    t.assert_error(ssl_certificate.register_discovery, nil)
end
