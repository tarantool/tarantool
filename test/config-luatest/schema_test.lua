...
local function find_variant(schema, predicate)
    for _, variant in ipairs(schema.variants) do
        if predicate(variant) then
            return variant
        end
    end
    return nil
...
local function variant_by_type(schema, schema_type)
    return find_variant(schema, function(variant)
        return variant.type == schema_type
    end)
end

local function record_variant_by_field(schema, field_name)
    return find_variant(schema, function(variant)
        return variant.type == 'record' and variant.fields[field_name] ~= nil
    end)
end

local function record_variant_by_field_path(schema, ...)
    local path = {...}
    return find_variant(schema, function(variant)
        local node = variant
        for _, field_name in ipairs(path) do
            if node.type ~= 'record' or node.fields[field_name] == nil then
                return false
            end
            node = node.fields[field_name]
        end
        return true
    end)
end

local function record_or_array_discriminator(data, w)
    if type(data) ~= 'table' then
        return nil
    end
    if data[1] ~= nil then
        return variant_by_type(w.schema, 'array')
    end
    return variant_by_type(w.schema, 'record')
end

local function foo_or_bar_record_discriminator(data, w)
    if type(data) ~= 'table' then
        return nil
    end
    if data.foo ~= nil then
        return record_variant_by_field(w.schema, 'foo')
    end
    if data.bar ~= nil then
        return record_variant_by_field(w.schema, 'bar')
    end
    return nil
end

local function kind_record_discriminator(data, w)
    if type(data) ~= 'table' then
        return nil
    end
    if data.kind == 'A' then
        return record_variant_by_field_path(w.schema, 'foo', 'a')
    end
    if data.kind == 'B' then
        return record_variant_by_field_path(w.schema, 'foo', 'b')
    end
    return nil
end
...
g.test_union_constructor = function()
    ...
    local discriminator = function() end
    t.assert_equals(schema.union({
        variants = {scalar_1, scalar_2},
    ...
    t.assert_equals((pcall(schema.union, {
        variants = {scalar_1, scalar_2},
        discriminator = 'foo',
    })), false)
...
g.test_validate_schema_union_discriminator = function()
    local exp_err_msg = '[myschema] A union without a discriminator ' ..
        'requires scalar variants'
    t.assert_error_msg_equals(exp_err_msg, function()
        schema.new('myschema', schema.union({
            variants = {
                schema.record({}),
                schema.array({items = schema.scalar({type = 'string'})}),
            },
        }))
    end)

    local exp_err_msg = '[myschema] A union without a discriminator ' ..
        'requires variants with distinct data types'
    t.assert_error_msg_equals(exp_err_msg, function()
        schema.new('myschema', schema.union({
            variants = {
...
g.test_validate_union_by_discriminator = function()
    local s = schema.new('myschema', schema.union({
        variants = {
            schema.scalar({
                type = 'string',
                ...
                validate = function(data, w)
                    if data:sub(1, 6) ~= 'uri://' then
                        w.error('URI must start with uri://')
                    end
                end,
            }),
            schema.scalar({
                ...
                kind = 'instance_name',
                validate = function(data, w)
                    if data:find(':', 1, true) ~= nil then
                        w.error('Instance name must not contain ":"')
                    end
                end,
            }),
        },
        discriminator = function(data, w)
            if type(data) ~= 'string' then
                return nil
            end
            if data:sub(1, 4) == 'uri:' then
                return find_variant(w.schema, function(variant)
                    return variant.kind == 'uri'
                end)
            end
            return find_variant(w.schema, function(variant)
                return variant.kind == 'instance_name'
            end)
        end,
    }))

    s:validate('uri://localhost:3301')
    s:validate('storage-001')

    assert_validate_error(s, 'uri:bad', table.concat({
        '[myschema] Data does not match selected union variant #1',
        '(variant #1: URI must start with uri://)',
    }, ' '))
end

g.test_validate_union_bad_discriminator_result = function()
    local s = schema.new('myschema', schema.union({
        variants = {
            schema.scalar({type = 'string'}),
            schema.scalar({type = 'number'}),
    ...
    assert_validate_error(s, 'foo',
        '[myschema] Union discriminator returned a schema node that is not ' ..
        'present in union variants')
end

g.test_validate_union_nonarray_variants = function()
    t.assert_error_msg_equals(
        '[myschema] Union variants must be an array',
        function()
            schema.new('myschema', {
                type = 'union',
                variants = {
...
g.test_get_index_union = function()
    local s = schema.new('myschema', schema.union({
        ...
        discriminator = foo_or_bar_record_discriminator,
    }))
...
g.test_get_index_union_uses_discriminator = function()
    local s = schema.new('myschema', schema.union({
        variants = {
            schema.record({
                kind = schema.scalar({type = 'string'}),
                foo = schema.record({
                    a = schema.scalar({type = 'number'}),
                }),
            }),
            schema.record({
                kind = schema.scalar({type = 'string'}),
                foo = schema.record({
                    b = schema.scalar({type = 'number'}),
                }),
            }),
        },
        discriminator = kind_record_discriminator,
    }))

    local data = {kind = 'B', foo = {b = 10}}
    t.assert_equals(s:get(data, 'foo.b'), 10)

    local exp_err_msg = '[myschema] foo.a: No such field in the schema'
    t.assert_error_msg_equals(exp_err_msg, function()
        s:get(data, 'foo.a')
    end)
end
...
g.test_set_index_union = function()
    ...
    local s_switch = schema.new('myschema_switch', schema.union({
        ...
        discriminator = function(data, w)
            ...
            if type(data.foo) == 'number' then
                expected_type = 'number'
            end
            return find_variant(w.schema, function(variant)
                return variant.fields.foo.type == expected_type
            end)
        end,
    }))
...
g.test_map_union = function()
    local s = schema.new('myschema', schema.union({
        ...
        discriminator = record_or_array_discriminator,
    }))
