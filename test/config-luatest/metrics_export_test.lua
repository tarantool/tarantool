...
local SSL_DIR = fio.abspath(fio.pathjoin('test', 'config-luatest', 'ssl_data'))
...
g.test_http_export_server_requires_roles_httpd = function()
    ...
    local exp_msg = 'failed to get server by name "additional", check that ' ..
                    'roles.httpd is already applied'
