...
g.test_prepared = function(cg)
    cg.server:exec(function(remote)
        ...
        if remote then
            ...
            cn = netbox.connect(box.cfg.listen, {user = 'test_user',
                                                 password = 'test'})
        ...
        t.assert_equals(unprepare(s.stmt_id), nil)
        ...
        t.assert_equals(unprepare(s2.stmt_id), nil)
        t.assert_equals(unprepare(s1.stmt_id), nil)
        ...
        if remote then
            cn:close()
            box.schema.user.revoke('test_user',
                                   'read, write, execute', 'universe')
