...
g.test_prepared = function(cg)
    cg.server:exec(function(remote)
        ...
        t.assert_equals(execute(s.stmt_id, {1, 2}).rows, {{1, 2, '3'}})
        t.assert_equals(execute(s.stmt_id, {1, 3}).rows, {})
        ...
        t.assert_equals(execute(s.stmt_id).rows, {{2}})
        t.assert_equals(execute(s.stmt_id).rows, {{2}})
        ...
        t.assert_equals(execute(s.stmt_id).rows, {{1, 1, '3', 1}})
        t.assert_equals(execute(s.stmt_id).rows, {{1, 1, '3', 1}})
        unprepare(s.stmt_id)
        ...
        t.assert_equals(execute(s.stmt_id, {'6'}).rows, {{5}})

        t.assert_equals(execute(s.stmt_id, {'9'}).rows, {{8.5}})
        unprepare(s.stmt_id)
        ...
        sql = "EXPLAIN QUERY PLAN SELECT a FROM test WHERE b = '3';"
        ...
        res = execute(s.stmt_id, {{[':a'] = 1}, {[':b'] = 2}, {[':c'] = 3}});
        t.assert_equals(res, exp)
        exp = {
            metadata = {
                {
        ...
        t.assert_equals(execute(s.stmt_id), exp)
        unprepare(s.stmt_id)
