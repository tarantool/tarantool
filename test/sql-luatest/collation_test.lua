...
g.test_3185_none_and_binary_collations = function(cg)
    cg.server:exec(function()
        ...
        t.assert_equals(res, coll_binary_id)
        box.execute("DROP TABLE t;")
        ...
        coll_binary_id = box.space._collation.index.name:get('binary').id
        t.assert_equals(res, coll_binary_id)
...
g.test_3644_foreign_key_for_unicode_ci_fails = function (cg)
    cg.server:exec(function ()
        ...
        box.execute([[CREATE TABLE t0 (s1 STRING
...
g.test_3573_collation_strength = function (cg)
    cg.server:exec(function ()
        ...
        local exp = {277, 'c', 1, 'ICU', 'unicode', {strength = "tertiary"}}
        t.assert_equals(res, exp)
