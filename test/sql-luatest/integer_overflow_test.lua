...
--
g.test_4015_convert_integer_to_float = function(cg)
    cg.server:exec(function(engine)
        box.execute("CREATE TABLE t(id INT PRIMARY KEY, a NUMBER);")
        local res = box.space.t:insert({1, 18446744073709551615ULL})
        t.assert_equals(res, {1, 18446744073709551615ULL})
        res = box.space.t:insert({2, -1})
        t.assert_equals(res, {2, -1})
        res = box.execute("SELECT * FROM t;")
        t.assert_equals(res.rows, {{1, 18446744073709551615ULL}, {2, -1}})
        box.space.t:drop()
    end)
end
