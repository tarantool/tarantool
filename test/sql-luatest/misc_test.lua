...
--
g.test_multistatement_queries = function(cg)
    cg.server:exec(function()
        ...
        exp_err = "At line 1 at or near position 39: keyword 'SELECT' "..
                  "is reserved. Please use double quotes "..
                  "if 'SELECT' is an identifier."
...
--
g.test_literals_metadata_type = function(cg)
    cg.server:exec(function()
        local exp = {
            metadata = {
                 {
...
--
g.test_exclude_null = function(cg)
    cg.server:exec(function()
        local sql = [[CREATE TABLE j (s1 INT PRIMARY KEY,
                      s2 STRING, s3 VARBINARY);]]
        box.execute(sql)
        local s = box.space.j
        local opts = {parts = {2, 'string', exclude_null = true}}
        box.space.j:create_index('I3', opts)
        ...
        local res = box.execute([[SELECT * FROM j;]])
