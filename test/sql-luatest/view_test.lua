...
g.test_view = function(cg)
    cg.server:exec(function()
        ...
        local exp_err = "Can't modify space 'v1': can not add index on a view"
        ...
        t.assert_error_msg_equals(exp_err, v1.create_index, v1, 'secondary', opts)
        ...
        t.assert_error_msg_equals(exp_err, box.space._space.replace,
                                  box.space._space, v1)

        local t1 = box.space._space.index[2]:select('t1')[1]:totable()
        t1[6]['view'] = true
        ...
        local raw_sp = box.space._space:get(sp.id):totable()
        sp:drop()
        raw_sp[6].sql = 'CREATE VIEW v as SELECT * FROM t1;'
        raw_sp[6].view = true
        sp = box.space._space:replace(raw_sp)
        local res = box.space._space:select(sp['id'])[1]['name']
        ...
        exp_err = "Can't drop space 't1': other views depend on this space"
        t.assert_equals(err.message, exp_err)
        sp = box.space._space:get{box.space.t1.id}
        ...
        local space_tuple = box.space._space.index[0]:max():totable()
        ...
        t.assert_error_msg_equals(exp_err, box.space._space.insert,
                                  box.space._space, space_tuple)
        box.space.bcv:drop()
...
g.test_4104_view_access_check = function(cg)
    cg.server:exec(function()
        ...
        box.execute([[INSERT INTO supersecret
                             VALUES(1, 'very very big secret');]])
        box.execute("INSERT INTO supersecret2 VALUES(1, 'very big secret 2');")
        box.execute([[CREATE VIEW leak
                            AS SELECT * FROM supersecret, supersecret2;]])
...
g.test_4740_instead_of_delete_update_check = function(cg)
    cg.server:exec(function()
        ...
        box.execute([[CREATE TRIGGER r1 INSTEAD OF DELETE ON v
                     FOR EACH ROW BEGIN INSERT INTO t1 VALUES (NULL, 1); END;]])
        box.execute([[CREATE TRIGGER r2 INSTEAD OF UPDATE ON v
                     FOR EACH ROW BEGIN INSERT INTO t1 VALUES (NULL, 2); END;]])
        local exp = {
