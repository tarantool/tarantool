...
int
is_interval(box_function_ctx_t *ctx, const char *args, const char *args_end)
{
	(void)args_end;
...
int
ret_interval(box_function_ctx_t *ctx, const char *args, const char *args_end)
{
	uint32_t arg_count = mp_decode_array(&args);
	if (arg_count != 1) {
		return box_error_set(__FILE__, __LINE__, ER_PROC_C,
