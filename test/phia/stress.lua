...
box.once("phia_stress", function()
    local s1 = box.schema.space.create('s1', { engine = 'phia', if_not_exists = true })
    s1:create_index('pk', {if_not_exists = true})

    local s2 = box.schema.space.create('s2', { engine = 'phia', if_not_exists = true })
    s2:create_index('pk', {compression = 'zstd', compression_branch = 'zstd', if_not_exists = true})

    local s3 = box.schema.space.create('s3', { engine = 'phia', if_not_exists = true })
