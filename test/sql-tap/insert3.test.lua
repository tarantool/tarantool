...
if (1 > 0)
 then
    ...
    for _, tab in ipairs(test:execsql("SELECT name FROM sqlite_master WHERE type = 'table'")) do
        test:execsql("DROP TABLE "..tab.."")
    end
    db("close")
    sqlite3("db", "test.db")
    ---------------------------------------------------------------------------
    ...
    test:do_test(
        ...
        function()
            ...
            for _ in X(0, "X!for", [=[["set ii 0","$ii < 10","incr ii"]]=]) do
                test:execsql(string.format("INSERT INTO t1 SELECT %s, %s, %s FROM t1", r, r, r))
            end
