...
g.before_all(function(g)
    ...
    g.server_1_uri = server.build_listen_uri("server1", g.replicaset.id)
    g.server_2_uri = server.build_listen_uri("server2", g.replicaset.id)
    g.server_3_uri = server.build_listen_uri("server3", g.replicaset.id)

    g.server1 = g.replicaset:build_and_add_server({
        alias = "server1",
        box_cfg = {
            listen = g.server_1_uri,
            replication = {g.server_1_uri, g.server_2_uri, g.server_3_uri},
...
g.test_box_wait_quorum_while_changing_replication_synchro_quorum = function(g)
    ...
    t.assert(g.server1:grep_log("ER_QUORUM_WAIT"))
end
