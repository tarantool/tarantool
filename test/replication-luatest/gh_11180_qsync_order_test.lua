...
g.test_cascading_rollback_from_journal = function(cg)
    cg.server:exec(function()
        ...
        box.cfg{replication_synchro_queue_max_size = 2000}
        local results = {}
        local function make_txn_fiber(id)
            return fiber.create(function()
                fiber.self():set_joinable(true)
        ...
        box.error.injection.set('ERRINJ_WAL_DELAY', false)
        t.assert_not((f1:join()))
        t.assert_not((f2:join()))
        t.assert_not((f3:join()))
        t.assert_equals(results, {'rollback 3', 'rollback 2', 'rollback 1'})
