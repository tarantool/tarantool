...
local g = t.group('gh-11157-accounting-booting-replicas-in-sync-quorum')
--
-- gh-11157:
-- Accounting booting replicas in sync quorum.
--
...
g.before_each(function(cg)
    ...
    -- Bootstrap master, booted, replicable
    ...
    -- Non-booted, replicable
    cg.replica2 = cg.cluster:build_and_add_server({
        alias = 'replica2',
        box_cfg = box_cfg,
    })
    cg.replica2_admin = fio.pathjoin(cg.replica2.workdir, 'replica2.admin')
    cg.replica2.env.TARANTOOL_RUN_BEFORE_BOX_CFG =
        -- Stop WAL to prevent the node booting
        "box.error.injection.set('ERRINJ_WAL_DELAY', true)\n" ..
        string.format(console_listen, cg.replica2_admin)

    -- Booted, non-replicable
    cg.replica3 = cg.cluster:build_and_add_server({
        alias = 'replica3',
        box_cfg = box_cfg,
    })
    cg.replica3.env.TARANTOOL_RUN_BEFORE_BOX_CFG =
        -- Stop the relay so that replica1 cannot synchronize with replica3
...
-- Booting replicas should not be counted in the sync quorum calculation
-- or when checking whether this quorum has been established
g.test_accounting_booting_replicas_in_sync_quorum = function(cg)
    wait_uuid(cg.master)

    local master_sock =
        t.helpers.retrying({}, connect_console, cg.master_admin)
    make_bootstrap_leader(master_sock)
    master_sock:close()

    -- Wait until the nodes get bootstraped
    cg.master:wait_until_ready()
    cg.replica3:wait_until_ready()

    -- Wait for replica1 to connect to replica3 before connecting to the
    -- bootstrap master to ensure that replica3 is counted in the sync quorum
    ...
    -- Shutdown
    local replica2_sock =
        t.helpers.retrying({}, connect_console, cg.replica2_admin)
    replica2_sock:write(
        "box.error.injection.set('ERRINJ_WAL_DELAY', false)\n",
        SOCKET_TIMEOUT)
    replica2_sock:close()

    cg.replica3:exec(function()
