...
g.test_accounting_booting_replicas_in_sync_quorum = function(cg)
    ...
    cg.replica2:wait_until_ready()

    -- Wait for replica1 to connect to replica2 before connecting to the
    -- bootstrap master to ensure that replica2 is counted in the sync quorum
    wait_applier_connected(cg.replica1, cg.replica2)
    cg.master_proxy:resume()
    -- Now cg.replica1 exits `replicaset_connect` with `state.connected = 2`
    -- so `replication_sync_quorum_auto` will be set to 2

    -- Wait until replica1 get bootstraped
    cg.replica1:wait_until_ready()
    -- Replica1 cannot synchronize with replica2,
