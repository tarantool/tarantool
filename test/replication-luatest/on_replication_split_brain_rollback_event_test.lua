                ...
                return function(it)
                    ...
                    for obj in it() do
                        _G.trigger_ok = pcall(function()
                            t.assert(_G.msgpack.is_object(obj))
                            local meta = obj.meta
                            local rid = meta.replica_id
                            local lsn = meta.lsn
                            local term = meta.term
                            local stmt = obj.statement
                            local sid = stmt.space_id
                            local type = stmt.op_type
                            local arg
                            if request_type == box.iproto.type.REPLACE or
                               request_type == box.iproto.type.INSERT or
                               request_type == box.iproto.type.UPSERT then
                                arg = stmt.tuple
                                t.assert_equals(stmt.key, nil)
                            else
                                arg = stmt.key
                            end
                            local ops
                            if request_type == box.iproto.type.UPDATE then
                                ops = stmt.tuple
                                t.assert_equals(stmt.ops, nil)
                            else
                                ops = stmt.ops
                            end
                            local old = stmt.old_tuple
                            local new = stmt.new_tuple
