...
--
-- The test covers the follow scenario:
...
-- 5. It waits for a quorum on both the txns.
-- 6. It writes PROMOTE and becomes a true leader.
--
-- The expected behaviour is that the node will wait for the both transactions
-- to gain the quorum. Not like it would only collect quorum for whatever
...
g.before_each(function(cg)
    ...
    wait_full_sync(cg, all_names)
    for _, name in pairs(all_names) do
        cg[name]:update_box_cfg({election_mode = 'manual'})
    end
...
g.test_new_synchronous_transactions_appeared_while_wait_quorum = function(cg)
    ...
    --
    -- The node2 must wait for the last txn (txn2) gain the quorum and write
