...
-- is in a certain state (bind or closed). It can help us in scenario in which
-- the certain socket is binded by another process of external test.
--
local function assert_socket_state(uri, state)
...
local function test_only_one_record_appears_in_logs(server, record, wait_time)
    ...
    t.helpers.retrying({timeout = 10}, function()
        first_log_record = server:grep_log(record, MAX_INT)
    ...
    local last_log_record = server:grep_log(record, MAX_INT)
...
g.test_no_spam_in_logs_while = function()
    ...
    assert_socket_state(storage_uri, "closed")
    ...
    local conn_refused_log_before_restart =
        g.router:grep_log(g.connection_refused_pattern, MAX_INT)
    g.storage:update_box_cfg({listen = storage_uri})
    g.storage:grep_log("entering the event loop")
    assert_socket_state(storage_uri, "bind")
