...
g.test_election_off_demote_other_same_term = function(g)
    ...
    -- The queue is still owned by server1. But none of the instances have the
    -- Raft leader role. Hence both read-only.
    --
    -- Previously in older versions server1 would be actually writable here.
    -- Because it was simply ignoring the Raft state machine.
    check_synchro_owner(g.server1, owner_id)
    check_is_ro(g.server1, true)
