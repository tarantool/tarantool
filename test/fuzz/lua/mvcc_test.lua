...
Usage: tarantool <path_to_luatest_binary> ./mvcc_test.lua
]]
...
local function generate_init_code(memtx_use_mvcc_engine)
    return ("os.execute('rm -rf *.snap *.xlog *.vylog 512 513')\n" ..
           "\n" ..
           ...
           "\n"):format(memtx_use_mvcc_engine and 'true' or 'false')
end

local function generate_schema_code()
    return "box.schema.space.create('m', {engine = 'memtx'})\n" ..
...
local function gen_key_or_null()
    if math.random() <= p_null_key then
        return "box.NULL"
    end
...
local function gen_keys(idx)
    local keys = {}
    for _ in ipairs(idx.fields) do
        table.insert(keys, gen_key())
    end
    return keys
end
...
local function gen_tuple()
    ...
    for _, field in ipairs(space_format) do
        if field.is_nullable then
            table.insert(tuple, gen_key_or_null())
        else
            table.insert(tuple, gen_key())
        end
    end
    return tuple
end
...
local function gen_update()
    return {gen_key_or_null(), gen_key_or_null()}
end
