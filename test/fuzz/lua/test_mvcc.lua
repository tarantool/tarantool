...
if params.help or params.h then
 ...
 a boolean option. The options list with default values:
   ...
   n_rounds <number, 1000>   - number of test execution rounds
   tx_cnt <number, 32>       - number of transactions in 1 round
   ro_tx_cnt <number, 8>     - number of read-only transactions in 1 round
   stmt_cnt <number, 320>    - number of transaction statements in 1 round
   p_rollback <number, 0.05> - probability of transaction rollback
   p_commit <number, 0.1>    - probability of transaction commit
   max_key <number, 8>       - maximum random unsigned key used (minimum is 1)
   p_null_key <number, 0.1>  - probability of NULL value for nullable keys
...
local N_ROUNDS = params.n_rounds or 1000
-- Number of transactions in 1 round.
local TX_CNT = params.tx_cnt or 32
-- Number of read-only transactions.
local RO_TX_CNT = params.ro_tx_cnt or 8
-- Number of transaction statements in 1 round.
local STMT_CNT = params.stmt_cnt or 320
-- Probability of rollback.
local P_ROLLBACK = params.p_rollback or 0.05
-- Probability of commit.
local P_COMMIT = params.p_commit or 0.1
-- Max random unsigned key (min is 1).
local MAX_KEY = params.max_key or 8
-- Probability of NULL value for nullable keys.
local P_NULL_KEY = params.p_null_key or 0.1
...
local serial
-- Server used for testing MVCC.
local mvcc = nil
-- Oracle server used for checking MVCC consistency.
local oracle = nil
...
box.space.m:create_index('s5', {parts = {
    {3, 'uint', is_nullable = true},
    {4, 'uint', is_nullable = true}}})
box.space.m:create_index('s6', {unique = false, parts = {
    {1, 'uint', is_nullable = false},
    {4, 'uint', is_nullable = true}}})
box.space.m:create_index('s7', {type = 'HASH', parts = {
    {1, 'uint'},
    {2, 'uint'}}})
box.space.m:create_index('s8', {parts = {
    {1, 'uint'},
    {2, 'uint'}}, func = 'func'})
...
local function gen_stmts()
    local txs = {}
    for tid = 1, TX_CNT do
        table.insert(txs, tx_new(mvcc.net_box, tid))
    end
...
local function try_apply_tx(tx_operations, rw)
    for i, operation in ipairs(tx_operations) do
        ...
        local _, res = pcall(function()
            return oracle:eval('return ' .. operation.str)
        end)
...
local function open_files()
    ...
    local repro_file_path = fio.pathjoin(mvcc.workdir, '..',
                                         repro_file_name)
    local err
    ...
    local serial_file_path = fio.pathjoin(oracle.workdir, '..',
        serial_file_name)
...
local function setup_test()
    ...
    oracle = server:new({alias = 'oracle'})
    mvcc = server:new({
        alias = 'mvcc',
        box_cfg = {
...
local function run_test()
    ...
    for _ = 1, N_ROUNDS do
        mvcc:eval(schema_creation_code)
        ...
        oracle:eval(schema_creation_code)
