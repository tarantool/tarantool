...
g.test_duplicate_key_error = function(cg)
    cg.server:exec(function(engine)
        local function check_err(space_name, index_name, tuple, key,
                                 old_pk, new_pk)
            ...
            t.assert_error_covers({
                ...
                index = index_name,
                key = key,
                old_tuple_primary_key = old_pk,
                new_tuple_primary_key = new_pk,
            }, space.insert, space, tuple)
        end
        ...
        check_err('test', 'pk', {1, 2}, {1}, {1}, {1})
        check_err('test', 'i1', {2, 1}, {1}, {1}, {2})
        t.assert_equals(s:select({}, {fullscan = true}), {{1, 1}})
        s.index.i1:drop()
        check_err('test', 'pk', {1, 2}, {1}, {1}, {1})
        check_err('test', 'i2', {2, 1}, {1}, {1}, {2})
        t.assert_equals(s:select({}, {fullscan = true}), {{1, 1}})
        s.index.i2:drop()
        check_err('test', 'pk', {1, 2}, {1}, {1}, {1})
        s:insert({2, 1})
