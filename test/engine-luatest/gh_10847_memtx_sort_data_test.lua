...
g_generic.test_multiple_spaces = function(cg)
    ...
    local test_log = 'Space \'test\' presorted indexes: done'
    local test2_log = 'Space \'test2\' presorted indexes: done'
    t.assert(cg.server:grep_log(test_log))
    t.assert(cg.server:grep_log(test2_log))
end
...
-- Test that checkpoint does not fail if a sort data file exists already.
g_generic.test_remove_snap_keep_sortdata = function(cg)
    cg.server:exec(function()
        ...
        -- Call box.snapshot() once again: .snap file recreated.
        box.snapshot()
        t.assert(#fio.glob(glob_snap) == 1)
        t.assert(#fio.glob(glob_sortdata) == 1)
    end)
