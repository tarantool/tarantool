...
g_generic.test_space_before_replace = function(cg)
    ...
    t.assert(cg.server:grep_log('Space \'test\' presorted indexes: done'))
end
...
g_generic.test_vinyl_space_ignored = function(cg)
    ...
    local memtx_log = 'Building presorted indexes in space \'test\'...'
    local vinyl_log = 'Building presorted indexes in space \'test_vinyl\'...'
    t.assert(cg.server:grep_log(memtx_log) ~= nil)
    t.assert(cg.server:grep_log(vinyl_log) == nil)
end
...
-- Test that the sort data file is recreated on box.snapshot after removal.
g_generic.test_remove_sortdata_keep_snap = function(cg)
    cg.server:exec(function()
        ...
        -- Call box.snapshot() once again: sort data file recreated.
        box.snapshot()
...
g_generic.test_remove_snap_keep_sortdata = function(cg)
    cg.server:exec(function()
        ...
        local fio = require('fio')
        local glob = fio.pathjoin(box.cfg.memtx_dir, '*.snap')
        local files = fio.glob(glob)
        t.assert(#files > 0) -- a sort data file must have been created
        for _, file in pairs(files) do
            fio.unlink(file)
        ...
        -- Can't box.snapshot: the sort data file exists already.
        local msg = '.sortdata\' already exists: File exists'
        t.assert_error_msg_contains(msg, box.snapshot)
    end)
end

-- Test that both snapshot and sort data file recreated if both are absent.
...
g_generic.test_snapshot_abort_discards_sortdata_file = function(cg)
    ...
    cg.server:exec(function()
        ...
        f:set_joinable(true)
        box.error.injection.set('ERRINJ_SORTDATA_WRITE_DELAY', true)

        -- Wait for the snapshot file to appear.
        local signature = box.info.signature
        local filename = string.format('%020d.sortdata', signature)
        repeat
            fiber.sleep(0.01)
        ...
        -- Kill the checkpoint fiber and see the sort data file removed.
        f:cancel()
        t.assert(not f:join())
        t.assert_equals(#fio.glob(glob), 0)

        -- Now do it successfully and see the newly created sort data file.
        box.error.injection.set('ERRINJ_SORTDATA_WRITE_DELAY', false)
        box.snapshot()
        t.assert_equals(#fio.glob(glob), 1)
    end)
end
