...
-- Do not call t.skip() from the top level of a wrapper test file: luatest
-- implements it as a special error, and during test file loading it is treated
-- as a bootstrap failure instead of a skipped test. If http.server is missing,
-- register a tiny test case that skips at execution time and let the wrapper
-- return before loading the upstream test module, which may require http.server
-- at the top level.
function M.skip_if_no_http_server()
    local has_http_server = pcall(require, 'http.server')
    ...
    local g = t.group()
    g.test_skip_if_no_http_server = function()
        t.skip('http.server module is not available')
    end
