...
# insert:
admin("box.process(box.net.box.REPLACE, box.pack('ip', 0, {1, 'testing', 'lua rocks'}))")
# select:
admin("box.process(box.net.box.SELECT, box.pack('iiiip', 0, 0, 0, 2^31, {1}))")
# delete:
admin("box.process(box.net.box.DELETE, box.pack('ip', 0, {1}))")
# check delete:
admin("box.process(box.net.box.SELECT, box.pack('iiiip', 0, 0, 0, 2^31, {1}))")
admin("box.process(box.net.box.CALL, box.pack('ii', 0, 0))")
...
admin("fifomax")
admin("s = box.space[0]")
admin("fifo_push(s, 1, 1)")
admin("fifo_push(s, 1, 2)")
admin("fifo_push(s, 1, 3)")
admin("fifo_push(s, 1, 4)")
admin("fifo_push(s, 1, 5)")
admin("fifo_push(s, 1, 6)")
admin("fifo_push(s, 1, 7)")
admin("fifo_push(s, 1, 8)")
admin("fifo_top(s, 1)")
admin("s:delete{1}")
admin("fifo_top(s, 1)")
admin("s:delete{1}")
...
"""
admin(lua.replace('\n', ' '))
admin("sort(box.crossjoin(0, 0, 0))")
admin("sort(box.crossjoin(0, 0, 10000))")
...
admin("box.fiber.resume(f)")
admin("box.fiber.resume(f)")
admin("function r() box.fiber.yield(box.space[0]:insert{0, 0, 1}) box.fiber.yield(box.space[0]:select{0}) box.fiber.yield(box.space[0]:truncate()) end")
admin("f = box.fiber.create(r)")
admin("box.fiber.resume(f)")
admin("box.fiber.resume(f)")
admin("box.fiber.resume(f)")
