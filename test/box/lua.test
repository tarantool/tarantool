...
admin("lua", simple=True)
admin("lua 1", simple=True)
admin("print('  lua says: hello')")
# What's in the box?
admin("local t = {} for n in pairs(box) do table.insert(t, '  - box.' .. tostring(n)) end table.sort(t) for i = 1, #t do print(t[i]) end t = nil")
# Test box.pack()
admin("box.pack()")
admin("box.pack(1)")
admin("box.pack('abc')")
admin("print(box.pack('a', ' - hello'))")
admin("print(box.pack('Aa', ' - hello', ' world'))")
admin("print(box.pack('w', 0x30))")
admin("print(box.pack('www', 0x30, 0x30, 0x30))")
admin("print(box.pack('www', 0x3030, 0x30))")
admin("print(string.byte(box.pack('w', 212345), 1, 2))")
admin("print(string.sub(box.pack('p', 1684234849), 2))")
admin("print(box.pack('p', 'this string is 45 characters long 1234567890 '))")
admin("print(box.pack('s', 0x4d))")
admin("print(box.pack('ssss', 25940, 29811, 28448, 11883))")
admin("print(box.pack('SSSS', 25940, 29811, 28448, 11883))")
admin("print(box.pack('SSSSSSSS', 28493, 29550, 27680, 27497, 29541, 20512, 29285, 8556))")
admin("print(box.pack('bsilww', 84, 29541, 1802444916, 2338318684567380014ULL, 103, 111))")
admin("print(box.unpack('b', 'T'))")
admin("print(box.unpack('s', 'Te'))")
admin("print(box.unpack('i', 'Test'))")
admin("print(box.unpack('l', 'Test ok.'))")
...
admin("print(box.unpack('i', num), str, box.unpack('l', num64))")
...
admin("fifo_top('test')")
admin("box.delete(0, 'test')")
admin("for k,v in pairs(box.cfg) do print(' - ', k, ': ', v) end")
admin("for k,v in pairs(box.space[0]) do if type(v) ~= 'table' then print(' - ', k, ': ', v) end end")
...
admin("t:next(t)")
admin("t:next(t:next())")
admin("for k, v in t:pairs() do print(v) end")
...
admin("for it,field in t:pairs() do print(field); end")
...
admin("function r(a, b) print(a) print(b) return a, b end")
