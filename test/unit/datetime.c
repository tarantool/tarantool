...
datetime_test(void)
{
	...
	for (index = 0; index < lengthof(tests); index++) {
		...
		is(date.epoch, date_strp.epoch,
		   "epoch:%g strp.epoch:%g delta:%g"
		   " reversible seconds via datetime_strptime for '%s'",
		   date.epoch, date_strp.epoch, date_strp.epoch - date.epoch,
		   buff);
		...
		is(date.epoch, date_parsed.epoch,
		   "epoch:%g parsed.epoch:%g delta:%g"
		   " reversible seconds via datetime_parse_full for '%s'",
		   date.epoch, date_parsed.epoch, date_parsed.epoch - date.epoch,
		   buff);
	}
	check_plan();
}
