...
/* Signal that the handler has been called. */
sem_t sem;
...
/**
 * The sem_wait call can be woken up by an interrupt, handle this.
 */
static void
sem_wait_noint(sem_t *sem)
{
	errno = 0;
...
static int
check_sender_pid_singlethread(void)
{
	plan(5);
	header();
	 ...
	 * can be less as some signals are probably merged on the system level.
	 */
	handler_call_count = 0;
	expect_sender_pid_self = true;
	...
	ok(handler_call_count > 0 &&
	   handler_call_count <= killer_thread_kill_count,
...
static int matched_pid_count;

static void
multithread_handler_f(int signum, siginfo_t *info, void *ctx)
{
	(void)signum;
	(void)ctx;
	fail_unless(signum == SIGUSR1);
	fail_unless(info->si_signo == signum);
	fail_unless(info->si_pid != 0);
	if ((info->si_pid == getpid()) == expect_sender_pid_self)
		matched_pid_count++;
	handler_call_count++;
	/* The first received PID must be valid. */
	if (handler_call_count == 1)
		fail_unless(matched_pid_count == 1);
	sem_post(&sem);
}

static void *
infinite_sleep_thread_f(void *arg)
{
	for (;;)
		thread_sleep(0.1);
	return NULL;
...
static int
check_sender_pid_muiltithread(void)
{
	plan(5);
	header();
	pthread_t killer_thread;
	...
	memset(&sa, 0, sizeof(sa));
	sa.sa_flags = SA_SIGINFO;
	sa.sa_sigaction = multithread_handler_f;
	fail_unless(tt_sigaction(SIGUSR1, &sa, NULL) == 0);
	...
	for (int i = 0; i < THREADS_NUM; ++i) {
		fail_unless(pthread_create(&child_threads[i], NULL,
					   infinite_sleep_thread_f, NULL) == 0);
	}
	...
	/*
	 * Test multiple signals from oneself. The amount of callbacks called
	 * can be less as some signals are probably merged on the system and
	 * the tt_sigaction level (during redirection).
	 *
	 * Cancel threads after sending signals to be sure all pending signals
	 * are handled. We can't just do the killer_thread_kill_count amount of
	 * sem_wait cause we can't be sure how many times the handler is called.
	 */
	handler_call_count = 0;
	expect_sender_pid_self = true;
	matched_pid_count = 0;
	...
	ok(handler_call_count <= killer_thread_kill_count,
	   "multiple signals (internal)");
	ok(matched_pid_count > 0 && matched_pid_count <= handler_call_count,
	   "the first redirected PID is valid, most of others probably too");
	 ...
	 *
	 * Cancel threads after sending the signals (see the comment above).
	 */
	handler_call_count = 0;
	expect_sender_pid_self = false;
	matched_pid_count = 0;
