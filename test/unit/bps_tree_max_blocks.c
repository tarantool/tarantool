...
static void
check_max_mem_required_for_inserts_case(size_t initial_size, size_t count)
{
	struct test tree;
	...
	fail_unless(test_size(&tree) == initial_size);

	/*
	 * Create a view, so any block modification will require a new block
	 * allocation. This maximizes the amount of required mem.
	 */
	struct test_view view;
	test_view_create(&view, &tree);

	size_t max_new_mem = test_max_mem_required_for_inserts(&tree, count);
	size_t max_mem = test_mem_used(&tree) + max_new_mem;

	fprintf(stderr, "Initial size: %lu\n", initial_size);
	fprintf(stderr, "Insertion count: %lu\n", count);
	fprintf(stderr, "Max footprint estimated: %lu\n", max_mem);
	fprintf(stderr, "Size prior to insertions: %lu\n", test_size(&tree));
	fprintf(stderr, "Footprint prior to insertions: %lu\n", test_mem_used(&tree));
	...
	fprintf(stderr, "Size after insertions: %lu\n", test_size(&tree));
	fprintf(stderr, "Footprint after insertions: %lu\n", test_mem_used(&tree));
...
static void
check_max_mem_required_for_inserts()
{
	plan(16);
	header();
	...
	for (unsigned i = 0; i < lengthof(height_max_size); i++) {
		for (unsigned j = 0; j < lengthof(height_max_size); j++) {
			check_max_mem_required_for_inserts_case(
				height_max_size[i], height_max_size[j]);
