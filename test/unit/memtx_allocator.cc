...
test_tuple_new(struct tuple_format *format, const char *data, const char *end)
{
	...
	struct tuple *tuple = memtx_block_get_tuple(block);
...
test_alloc_block()
{
	...
	is(memtx_block_get_data(block), (char *)block + 14, "data offset");
...
test_alloc_block_aligned()
{
	...
	block = MemtxAllocator<SmallAlloc>::alloc_aligned(0, 0);
	is(block, nullptr, "alignment 0 is not allowed");
	...
	is((uintptr_t)memtx_block_get_data(block) & 1, 0, "alignment 2");
	MemtxAllocator<SmallAlloc>::free(block);

	block = MemtxAllocator<SmallAlloc>::alloc_aligned(222, 3);
	is(block, nullptr, "alignment 3 is not allowed");

	block = MemtxAllocator<SmallAlloc>::alloc_aligned(333, 4);
	is((uintptr_t)memtx_block_get_data(block) & 3, 0, "alignment 4");
	MemtxAllocator<SmallAlloc>::free(block);

	block = MemtxAllocator<SmallAlloc>::alloc_aligned(444, 5);
	is(block, nullptr, "alignment 5 is not allowed");

	block = MemtxAllocator<SmallAlloc>::alloc_aligned(0, 8);
	is((uintptr_t)memtx_block_get_data(block) & 7, 0, "alignment 8");
	MemtxAllocator<SmallAlloc>::free(block);

	block = MemtxAllocator<SmallAlloc>::alloc_aligned(555, 64);
	is((uintptr_t)memtx_block_get_data(block) & 63, 0, "alignment 64");
