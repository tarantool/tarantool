...
do
    ...
    local status, err = pcall(function()
        test:test("fill and test cat output", function(test_i)
            ...
            check_ctlcat_xlog(test_i, dir, "--from=2 --to=5 --format=json", "\n", 2)
            check_ctlcat_xlog(test_i, dir, "--from=2 --to=5 --format=json --show-system", "\n", 3)
            check_ctlcat_xlog(test_i, dir, "--from=5 --to=2 --format=json --show-system", "\n", 0)
            check_ctlcat_xlog(test_i, dir, "--from=2 --to=5 --format=json --show-system --replica 1", "\n", 3)
            check_ctlcat_xlog(test_i, dir,
                "--from=2 --to=5 --format=json --show-system --replica 1 --replica 2", "\n", 3)
            check_ctlcat_xlog(test_i, dir, "--from=2 --to=5 --format=json --show-system --replica 2", "\n", 0)
...
if test_run == nil then
    ...
    local status, err = pcall(function()
        test:test("fill and test play output", function(test_i)
            ...
            test_i:is(lsn_before, 3, "check lsn before")
            local res, _, _ = run_command(dir, command_base)
            test_i:is(res, 0, "execution result")
            test_i:is(test_run:get_lsn("remote", 1), 9, "check lsn after")
            local res, _, _ = run_command(dir, command_base)
            test_i:is(res, 0, "execution result")
            test_i:is(test_run:get_lsn("remote", 1), 15, "check lsn after")
        end)
    end)
