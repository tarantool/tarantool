...
public class ByteUtil {
    ...
    public static byte[] convertLongIdToByteArray(long id) {
        final byte[] byteId = new byte[LENGTH_LONG];
    ...
    public static int decodeLengthInVar32Int(byte[] in, int offset) {
        ...
        if ((in[offset + 1] & 0x80) == 0) {
            return (in[offset] & 0x7F) << 7 | (in[offset + 1] & 0x7F);
        }
        if ((in[offset + 2] & 0x80) == 0) {
            return (in[offset] & 0x7F) << 14 | (in[offset + 1] & 0x7F) << 7
                    | (in[offset + 2] & 0x7F);
        }
        if ((in[offset + 3] & 0x80) == 0) {
            return (in[offset] & 0x7F) << 21 | (in[offset + 1] & 0x7F) << 14
                    | (in[offset + 2] & 0x7F) << 7 | (in[offset + 3] & 0x7F);
        }
        if ((in[offset + 4] & 0x80) == 0) {
            return (in[offset] & 0x7F) << 28 | (in[offset + 1] & 0x7F) << 21
                    | (in[offset + 2] & 0x7F) << 14
                    | (in[offset + 3] & 0x7F) << 7 | (in[offset + 4] & 0x7F);
        }
        return 0;
    }
     ...
     * 
     * @param out
     *            an <code>byte[]</code> to which the length is encoded.
     * @param offset
     *            position in out where start to write length
     * @param length
    ...
    public static int readBytes(byte[] src, int position, byte[] dest,
            int destPosition, int length) {
    ...
    public static byte[] toBytes(int value) {
        final byte[] bytes = new byte[LENGTH_LONG];
    ...
    public static int writeBytes(byte[] bytes, int position, byte[] value,
    ...
    public static int writeUnsignedInteger(byte[] bytes, int position,
            long value) {
        bytes[position + 3] = (byte) ((value >>> 24) & 0xFF);
        bytes[position + 2] = (byte) ((value >>> 16) & 0xFF);
        bytes[position + 1] = (byte) ((value >>> 8) & 0xFF);
