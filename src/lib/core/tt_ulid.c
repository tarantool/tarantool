...
{
	struct errinj *e = errinj(ERRINJ_ULID_TIME_FREEZE, ERRINJ_BOOL);
	uint64_t ms;

	if (e != NULL && e->bparam && last_ms != 0) {
		ms = last_ms;
	} else {
		/** Real Unix time in milliseconds. */
		uint64_t now_ms = (uint64_t)(clock_realtime() * 1000);
...
tt_ulid_from_string(const char *in, struct tt_ulid *u)
{
	...
	if (len != ULID_STR_LEN)
		return 1;
	return base32_crockford_decode(in, u->bytes, ULID_LEN);
}
