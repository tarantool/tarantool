		 ...
		 const char **ret_data_end)
{
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowArrayViewInitFromSchema failed: %s",
			 error.message);
		return -1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowArrayViewSetArray failed: %s",
			 error.message);
		goto error1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowIpcOutputStreamInitBuffer failed");
		goto error1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError, "ArrowIpcWriterInit failed");
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowIpcWriterWriteSchema failed: %s", error.message);
		goto error2;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowIpcWriterWriteArrayView failed: %s",
			 error.message);
		goto error2;
	}
		 ...
		 const char *data, const char *data_end)
{
	...
	if (size <= 0) {
		diag_set(SerializationError, "Unexpected data size");
		return -1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError, "ArrowBufferAppend failed");
		ArrowBufferReset(&buffer);
		return -1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowIpcInputStreamInitBuffer failed");
		ArrowBufferReset(&buffer);
		return -1;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowIpcArrayStreamReaderInit failed");
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowArrayStreamGetSchema failed: %s", error.message);
		goto error;
	}
	...
	if (rc != NANOARROW_OK) {
		diag_set(SerializationError,
			 "ArrowArrayStreamGetNext failed: %s", error.message);
