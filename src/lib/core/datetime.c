...
#if 0
#define DT_DEBUG(fmt, ...) printf("DEBUG %s: " fmt "\n", __func__, __VA_ARGS__)
#else
#define DT_DEBUG(fmt, ...)
#endif
...
local_dt(int64_t secs)
{
	...
	DT_DEBUG("rd_sec64:%ld rd_num64:%ld (int):%d",
		 secs, rd_number, (int)rd_number);
...
tm_to_datetime(struct tnt_tm *tm, struct datetime *date)
{
	...
	DT_DEBUG("{sec=%d, min=%d, hour=%d,"
		 " mday=%d, mon=%d, year=%d,"
		 " wday=%d, yday=%d, isdst=%d,"
		 " gmtoff=%ld, tzindex=%d, epoch=%ld, nsec=%d}",
