...
/**
 * Sort data file entry format string: space ID / index ID, phisical offset of
 * the corresponding data in the file, phisical size of the data, tuple count.
 *
...
static int
writer_print(struct memtx_sort_data_writer *writer, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
				   ...
				   struct read_view *rv)
{
	...
	if (writer_get_offset(writer, &entries_offset) != 0 ||
	    writer_print(writer, ENTRIES_FMT, 0) != 0)
		return -1;
	...
	read_view_foreach_space(space_rv, rv) {
		...
		for (uint32_t i = 0; i <= space_rv->index_id_max; i++) {
			...
			writer_print(writer, ENTRY_FMT, 0, 0, 0L, 0L, 0L);
			entry_count++;
		}
	}
	...
	if (writer_seek(writer, entries_offset, SEEK_SET) != 0)
		return -1;
	return writer_print(writer, ENTRIES_FMT, entry_count);
}
...
memtx_sort_data_writer_commit(struct memtx_sort_data_writer *writer)
{
	...
	if (writer_seek(writer, entry->header_entry_offset, SEEK_SET) != 0 ||
	    writer_print(writer, ENTRY_FMT, entry->key.space_id,
