		    ...
		    struct tt_uuid *instance_uuid)
{
	...
	read_view_foreach_space(space_rv, rv) {
		...
		for (uint32_t i = 1; i <= space_rv->index_id_max; i++) {
			...
			/*
			 * We only read-view SKs that support recovering using
			 * sort data. These must also support dumping the data.
			 */
			struct memtx_index_read_view *mirv =
				(struct memtx_index_read_view *)
					space_rv->index_map[i];
			assert(mirv->dump_sort_data != NULL);
			(void)mirv;
...
memtx_sort_data_create(struct memtx_sort_data *msd)
{
	...
	if ((msd->cardinality_offset = ftell(msd->fp)) == -1)
		goto fail;
	...
	mh_foreach(msd->entries, i) {
		...
		fprintf(msd->fp, "%u/%u: ", entry->key.space_id,
			entry->key.index_id);
		if ((entry->offset_offset = ftell(msd->fp)) == -1)
			goto fail;
		fprintf(msd->fp, "%016llx, ", 0LLU);
		if ((entry->psize_offset = ftell(msd->fp)) == -1)
			goto fail;
		fprintf(msd->fp, "%016llx, ", 0LLU);
		if ((entry->len_offset = ftell(msd->fp)) == -1)
			goto fail;
...
{
	if (msd->fp == NULL)
		return; /* Discard prior to creation. */
		      ...
		      uint32_t space_id, uint32_t index_id)
{
	...
	entry->offset = ftell(msd->fp); /* TODO: error handling? */
