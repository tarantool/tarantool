...
{
	return xdir_touch_xlog(&msd->dir, vclock);
}
				   ...
				   struct vclock *vclock)
{
	snprintf(msd->fname, sizeof(msd->fname), "%s", xdir_format_filename(
			&msd->dir, vclock_sum(vclock), INPROGRESS));
...
fail:
	memtx_sort_data_writer_discard_file(msd);
	return -1;
}
...
{
	assert(msd->fname[0] != '\0');
	return xlog_materialize_filename(msd->fname);
}
				   ...
				   uint32_t space_id, bool *has_sort_data)
{
	...
	struct memtx_sort_data_key key = {space_id, 0};
	mh_int_t i = mh_memtx_sort_data_entries_find(msd->entries, key, NULL);
	*has_sort_data = i != mh_end(msd->entries);
	if (*has_sort_data) {
		struct memtx_sort_data_entry *entry =
			mh_memtx_sort_data_entries_node(msd->entries, i);
		if (entry->len != 0) {
