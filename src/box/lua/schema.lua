...
--
-- The ibuf is used to encode a position given as table or tuple. The function
-- is expected to be called after a key is encoded on the same ibuf, and new
-- allocation can cause ibuf reallocation and invalidate the old pointers.
-- That's why the caller must pass `key` and `key_end` arguments to this
-- function and update them with returned "fixed" pointers.
--
-- Returns three values:
-- 1. Whether operation was succsessful. Is true on success. On failure,
--    is false and box.error is set.
-- 2. Fixed pointer to the beginning of the search key.
-- 3. Fixed pointer to the end of the search key.
--
local function iterator_pos_set(index, pos, ibuf, level, key, key_end)
    ...
    elseif type(pos) == 'string' then
        ...
        tuple_encode(ibuf, pos, level + 1)
        -- Set pointers according to ibuf contents:
        -- ibuf.rpos/key      key_end/tuple     ibuf.wpos/tuple_end
        -- V                        V                             V
        -- [......................................................]
        --
        -- Do not forget to update `key` pointers - they could be invalidated
        -- on ibuf reallocation during the last `tuple_encode` call.
        key = ibuf.rpos
        key_end = key + key_size
        local tuple = key_end
        local tuple_end = ibuf.wpos
...
base_index_mt.pairs_ffi = function(index, key, opts)
    ...
    local ok
    ok, pkey, pkey_end = iterator_pos_set(index, after, ibuf, 2, pkey, pkey_end)
...
base_index_mt.select_ffi = function(index, key, opts)
    ...
    local region_svp = builtin.box_region_used()
    local ok
    ok, key, key_end = iterator_pos_set(index, after, ibuf, 2, key, key_end)
    if ok then
