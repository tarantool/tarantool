...
--
-- Create a recovery point now via the backend. The id is composed by the
-- manager. Returns the backend payload, or nil + err. Does not interrupt the
-- scheduling loop.
--
local function manager_create_point(manager, opts)
    local point_id
    -- box.info is absent in an application thread.
    local info = box.info
    local instance = info ~= nil and (info.name ~= box.NULL and info.name
        or info.uuid ~= uuid.NULL and info.uuid)
    if instance then
        point_id = string.format('%s.%s.%s', instance, manager.name, uuid.str())
    else
        point_id = string.format('%s.%s', manager.name, uuid.str())
    end
    opts = opts or {}
    opts.timeout = opts.timeout or manager.timeout
    return manager.backend_instance:create_point(point_id, opts)
end
