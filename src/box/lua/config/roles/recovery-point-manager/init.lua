...
local config_schema = schema.new('recovery-point-manager', schema.record({
    ...
    validate = function(data, w)
        local managers = data.managers or {default = {}}
        for name, manager in pairs(managers) do
            manager = type(manager) == 'table' and manager or {}
...
local function effective_managers(cfg)
    cfg = cfg or {}
    -- managers omitted (or null): a single default manager.
    local managers = type(cfg.managers) == 'table'
        and cfg.managers or {default = {}}
    local defaults = {
        backend = cfg.backend,
    ...
    local res = {}
    for name, manager in pairs(managers) do
        -- An empty manager (`cs_1:`) arrives as box.NULL; treat it as {}.
        local options = type(manager) == 'table' and manager or {}
        ...
        local backends_cfg = eff.backends_cfg or {}
        local create = eff.create or {}
        res[name] = {
            backend_type = backend_type,
            backend_cfg = backend_type ~= nil and
                          backends_cfg[backend_type] or {},
            create_interval = create.by and create.by.interval,
...
local function validate(cfg)
    ...
    if type(cfg.managers) == 'table' then
        local managers = {}
        for name, manager in pairs(cfg.managers) do
            managers[name] = type(manager) == 'table' and manager or {}
        end
        cfg = table.copy(cfg)
