...
-- Create a recovery point on the target's leader. Returns the created point, or
-- nil + err on failure (call_leader raises, so wrap it) -- the manager retries.
local function instance_create_point(instance, id, opts)
    local ok, res = pcall(instance.rs.call_leader, instance.rs,
                          'box.backup.recovery_point.create', {id},
