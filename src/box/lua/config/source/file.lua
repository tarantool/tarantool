...
function methods.sync(self, config_module, _iconfig)
    ...
    for config_path in config_paths:iterator() do
        local config_abspath = fio.abspath(config_path)

        -- Prevent recursion.
        if processed_paths[config_abspath] then
            log.warn('skipping already processed config file: %q',
                     config_abspath)
            goto continue
        end
        processed_paths[config_abspath] = true

        log.debug('processing config file %q', config_abspath)
        config = read_config_file(config_abspath)
        ...
        if config.include ~= nil then
            local current_config_dir = fio.dirname(config_abspath)
            ...
            for _, glob in ipairs(config.include) do
                ...
                glob = file.rebase_file_path(current_config_dir, glob)
