...
I['failover.replicasets.*.mode'] = format_text([[
    The coordinator supports two leader appoint modes: `qsync` and
    `async`.
    ...
    Technically a user can enable the `qsync` mode and mark only
    some spaces synchronous. However, this setup doesn't give the
    best of two worlds:
    ...
    In this description we assume either `async` with all
    asynchronous spaces or `qsync` with all synchronous ones.

    The `async` mode handles a replicaset with asynchronous
    replication: if all the spaces are created without the
    `is_sync` option. It is default.

    The `qsync` mode handles a replicaset with quorum synchronous
    replication: if the spaces have the `is_sync` option enabled.
    ...
    *autofailover*: it *appoints* the new leader (it goes to
    ...
    about the longest available journal.
    ...
    *conflict detection* and the *conflict resolution* strategies.
    ...
    partitioning situations and it adds some delay for resigning
    ...
    - The old leader resignes after failover.lease_interval.
    ...
    [^1]: It works in assumption that the synchro queue owner has
          ...
          replication.failover = election in the past). It can be
          verified using `box.info.synchro.queue.owner`: it is
    ...
    `qsync` works this way:

    - After the network partitioning, the old leader resignes.
    ...
    - The conflict resolution strategy is the same as for the
      `async` mode.

    [^4]: Technically speaking, <...>.mode = qsync enables
          box.cfg.election_mode = manual on instances. It tunes
          the box.ctl.promote() behavior to check the instance's
          vclock with a quorum of replicas before actual write
          itself as the new synchro queue owner.
    [^5]: By calling box.ctl.promote().
    [^6]: If all the spaces are is_sync = true. So, again, mixing
          of async and sync spaces within one replicaset is not a
          good idea.
    ...
    Note that the `qsync` mode doesn't support a setup with two
    availability zones for now. You need at least three zones for
    it. Future tarantool versions may provide the two zones setup
    support. Stay tuned!
]])
