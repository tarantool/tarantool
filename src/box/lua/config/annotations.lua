...
M['<uri>.params.ssl_ca_file'] = toline([[
...
M['<uri>.params.ssl_cert_file'] = toline([[
    A path to an SSL certificate file: For a server, it's mandatory.
    For a client, it's mandatory if the ssl_ca_file parameter is set for a
    server; otherwise, optional.
]])

M['<uri>.params.ssl_ciphers'] = toline([[
...
M['<uri>.params.ssl_key_file'] = toline([[
    A path to a private SSL key file: For a server, it's mandatory. For a
    client, it's mandatory if the `ssl_ca_file` parameter is set for a server;
    otherwise, optional. If the private key is encrypted, provide a password
    for it in the `ssl_password` or `ssl_password_file` parameter
]])

M['<uri>.params.ssl_password'] = toline([[
    ...
    `ssl_password_file`. Tarantool applies the `ssl_password` and
    `ssl_password_file` parameters in the following order: If `ssl_password` is
    provided, Tarantool tries to decrypt the private key with it. If
    `ssl_password` is incorrect or isn't provided, Tarantool tries all passwords
    from `ssl_password_file` one by one in the order they are written.
    If `ssl_password` and all passwords from `ssl_password_file` are incorrect,
    or none of them is provided, Tarantool treats the private key as
    unencrypted.
]])

M['<uri>.params.ssl_password_file'] = toline([[
...
-- SM
M['app.cfg.*'] = toline([[
    `file` (a path to a Lua file to load an application from) or
...
M['audit_log.extract_key'] = toline([[
...
M['audit_log.filter'] = 'Enable logging for a specified subset of audit events.'

-- DM
-- SM
M['audit_log.filter.*'] = toline([[
...
M['audit_log.spaces.*'] = toline([[
    ...
    to monitor.
    Example: `spaces: [bands, singers]`, only the events of `bands` and
    `singers` spaces are logged:
]])

M['audit_log.to'] = 'Enable audit logging and define the log location.'

M['audit_log.syslog'] = toline([[
...
M['audit_log.syslog.facility'] = toline([[
...
M['compat'] = 'The compat section defines values of the compat module options.'

M['compat.binary_data_decoding'] = toline([[
    Define how to store binary data fields in Lua after decoding:
    `new`: as varbinary objects,
    `old`: as plain strings.
]])

M['compat.box_cfg_replication_sync_timeout'] = toline([[
...
-- DM
-- src/lua/compat.lua:140
M['compat.box_consider_system_spaces_synchronous'] = toline([[
...
M['compat.box_error_serialize_verbose'] = toline([[
...
M['compat.box_error_unpack_type_and_code'] = toline([[
...
M['compat.box_space_execute_priv'] = toline([[
...
M['compat.box_space_max'] = toline([[
    Set the maximum space identifier (`box.schema.SPACE_MAX`):
    `new`: 2147483646,
    `old`: 2147483647.
]])

M['compat.box_tuple_extension'] = toline([[
...
M['compat.console_session_scope_vars'] = toline([[
    Whether a console session has its own variable scope. In the old behavior,
    all the non-local variable assignments from the console are written to
    globals. In the new behavior, they're written to a variable scope attached
...
M['compat.sql_seq_scan_default'] = toline([[
...
-- DM
-- src/lua/compat.lua:150
M['compat.wal_cleanup_delay_deprecation'] = toline([[
...
M['compat.yaml_pretty_multiline'] = toline([[
...
M['conditional'] = toline([[
    This section defines the configuration parts that apply to
...
M['conditional.*'] = toline([[
    ...
    if specified conditions are met. Conditions can include `tarantool_version`
    and support comparison operators (`>`, `<`, `>=`, `<=`, `==`, `!=`), as
    well as logical operators (`||`, `&&`) and parentheses for grouping.
]])
...
M['conditional.*.*'] = toline([[
    ...
    options applied when the condition is met. Each condition compares the
    `tarantool_version` using operators (`<`, `>`, `<=`, `>=`, `==`, `!=`)
    and supports defining labels or other options that are applied only if
...
M['config.context.*.rstrip'] = toline([[
...
M['config.etcd'] = toline([[
    (EE) This section describes options related to providing connection
...
M['config.etcd.endpoints'] = toline([[
    The list of endpoints used to access an etcd server.
]])

M['config.etcd.endpoints.*'] = toline([[
    etcd endpoint name. For example: `http://localhost:2379`.
]])

-- DM
-- SM
M['config.etcd.http'] = 'HTTP request configuration.'

-- DM
-- SM
M['config.etcd.http.request'] = toline([[
    Option for the HTTP request configuration. Contains settings for HTTP
    requests, including request timeouts and socket connections.
]])

M['config.etcd.http.request.timeout'] = toline([[
    A time period required to process an HTTP request to an etcd server:
    from sending a request to receiving a response.
]])
M['config.etcd.http.request.unix_socket'] = toline([[
    A Unix domain socket used to connect to an etcd server.
]])

M['config.etcd.password'] = 'A password used for authentication.'

M['config.etcd.prefix'] = toline([[
...
-- DM
-- SM
M['config.etcd.ssl'] = toline([[
    Option for SSL configuration.
    Contains settings for configuring SSL, including paths to
    certificates, keys, and options for verifying peer and host during
    secure communication.
]])

M['config.etcd.ssl.ca_file'] = toline([[
    A path to a trusted certificate authorities (CA) file.
]])

M['config.etcd.ssl.ca_path'] = toline([[
...
M['config.etcd.ssl.verify_host'] = toline([[
    Enable verification of the certificate's name (CN) against
    the specified host.
]])

M['config.etcd.ssl.verify_peer'] = toline([[
...
-- DM
M['config.etcd.watchers'] = toline([[
    Option for configuring watcher behavior.
    Contains settings related to reconnection policies, including the timeout
    and maximum number of reconnection attempts when a watcher loses connection.
]])

M['config.etcd.watchers.reconnect_max_attempts'] = toline([[
    The maximum number of attempts to reconnect to an etcd server in case
    of connection failure.
]])

M['config.etcd.watchers.reconnect_timeout'] = toline([[
...
M['config.storage'] = toline([[
    (EE) This section describes options related to providing connection
    settings to a centralized Tarantool-based storage.
]])

M['config.storage.endpoints'] = toline([[
...
M['config.storage.endpoints.*.login'] = toline([[
...
M['config.storage.endpoints.*.uri'] = toline([[
    A URI of the configuration storage's instance.
]])

M['config.storage.prefix'] = toline([[
...
M['config.storage.reconnect_after'] = toline([[
    A number of seconds to wait before reconnecting to a configuration storage.
]])

M['config.storage.timeout'] = toline([[
    The interval (in seconds) to perform the status check of a
...
M['console'] = toline([[
    Configure the administrative console. A client to the console
    is `tt` connect.
]])

M['console.enabled'] = toline([[
...
M['console.socket'] = toline([[
    The Unix socket for the administrative console. Mind the following nuances:
    1. Only a Unix domain socket is allowed. A TCP socket can't be configured
    this way. 2.console.socket is a file path, without any `unix:` or `unix/:`
    prefixes. 3. If the file path is a relative path, it is interpreted relative
    to `process.work_dir`.
]])
...
M['credentials.roles.*.privileges.*.functions'] = toline([[
...
M['credentials.roles.*.privileges.*.lua_call'] = toline([[
...
-- DM
-- SM
M['credentials.roles.*.privileges.*.universe'] = toline([[
    Grants global permissions across all object types in the database,
    including: `read` (Read any object), `write` (Modify any object),
    `execute` (Execute functions or code), `session` (Connect via IPROTO),
    `usage` (Use granted privileges), `create` (Create users, roles, objects),
    `drop` (Remove users, roles, objects), `alter` (Modify settings or objects).
]])
...
M['credentials.users.*.password'] = 'A user\'s password.'

M['credentials.users.*.privileges'] = toline([[
...
M['database.hot_standby'] = toline([[
    ...
    repeats every `wal.dir_rescan_delay` seconds. For spaces created with engine
    set to `vinyl`.
]])

M['database.instance_uuid'] = toline([[
    An instance UUID. By default, instance UUIDs are generated automatically.
    `database.instance_uuid` can be used to specify an instance identifier
    manually. UUIDs should follow these rules:
    ...
    the system is restarted. The values must comply with RFC 4122.
    The nil UUID is not allowed.
]])

M['database.mode'] = toline([[
    An instance's operating mode. This option is in effect if
    `replication.failover` is set to off. The following modes are available:
...
M['database.replicaset_uuid'] = toline([[
    A replica set UUID. By default, replica set UUIDs are generated
    automatically. `database.replicaset_uuid` can be used to specify a replica
    set identifier manually.
]])

M['database.txn_isolation'] = 'A transaction isolation level.'

M['database.txn_timeout'] = toline([[
...
M['failover.probe_interval'] = toline([[
...
M['failover.renew_interval'] = toline([[
...
-- DM
-- src/box/lua/config/instance_config.lua:2353
M['failover.replicasets.*'] = 'Name of the replicaset.'

-- DM
-- src/box/lua/config/instance_config.lua:2358
M['failover.replicasets.*.priority'] = toline([[
...
M['failover.stateboard.keepalive_interval'] = toline([[
...
M['failover.stateboard.renew_interval'] = toline([[
...
M['feedback.crashinfo'] = toline([[
    Whether to send crash information in the case of an instance failure.
    This information includes: general information from the uname output, build
    information, the crash reason, the stack trace. To turn off sending crash
    information, set this option to false.
]])

M['feedback.enabled'] = toline([[
...
M['feedback.metrics_collect_interval'] = toline([[
    The interval (in seconds) for collecting metrics.
]])

M['feedback.metrics_limit'] = toline([[
...
M['feedback.send_metrics'] = toline([[
...
M['fiber.io_collect_interval'] = toline([[
    The time period (in seconds) a fiber sleeps between iterations of the
    event loop. `fiber.io_collect_interval` can be used to reduce CPU load
    in deployments where the number of client connections is large, but
    requests are not so frequent (for example, each connection issues just
    a handful of requests per second).
]])

M['fiber.slice'] = toline([[
...
M['fiber.slice.err'] = toline([[
    Set a time period (in seconds) that specifies the warning slice.
]])

M['fiber.slice.warn'] = toline([[
    Set a time period (in seconds) that specifies the error slice.
]])

M['fiber.too_long_threshold'] = toline([[
    If processing a request takes longer than the given period (in seconds),
    the fiber warns about it in the log. `fiber.too_long_threshold` has effect
    only if `log.level` is greater than or equal to 4 (`warn`).
]])
...
M['fiber.top.enabled'] = toline([[
    Enable or disable the `fiber.top()` function. Enabling `fiber.top()`
    slows down fiber switching by about 15%, so it is disabled by default.
]])

M['fiber.worker_pool_threads'] = toline([[
...
M['flightrec'] = toline([[
    (EE) The flightrec section describes options related to the flight
    recorder configuration.
]])

M['flightrec.enabled'] = 'Enable the flight recorder.'

M['flightrec.logs_log_level'] = toline([[
...
M['flightrec.metrics_period'] = toline([[
...
M['iproto.advertise'] = toline([[
    URIs for clients to let them know where to connect.
]])

M['iproto.advertise.client'] = toline([[
    ...
    1. An address: `host:port`. 2. A Unix domain socket: `unix/:`.
...
M['iproto.advertise.peer'] = toline([[
...
M['iproto.advertise.peer.login'] = toline([[
...
M['iproto.advertise.peer.password'] = toline([[
...
M['iproto.advertise.peer.uri'] = toline([[
...
M['iproto.advertise.sharding.password'] = toline([[
    (Optional) A password for the specified user. If a login is specified but
    a password is missing, it is taken from the user's credentials.
]])

M['iproto.advertise.sharding.uri'] = toline([[
    (Optional) A URI used to advertise the current instance. By default,
    the URI defined in iproto.listen is used to advertise the current instance.
]])
...
M['iproto.listen.*'] = toline([[
    Uri params. For example `- uri: '127.0.0.1:3301'`. Note that a URI value
    can't contain parameters, a login, or a password.
]])
...
M['iproto.net_msg_max'] = toline([[
    ...
    fibers handle, so that some pending requests are blocked. On powerful
    systems, increase net_msg_max, and the scheduler starts processing pending
    requests immediately. On weaker systems, decrease net_msg_max, and the
    overhead may decrease. However, this may take some time because the
    scheduler must wait until already-running requests finish. When
    `net_msg_max` is reached, Tarantool suspends processing of incoming
    ...
    restricts the number of incoming network messages that the transaction
    processor thread handles, and therefore indirectly affects the fibers that
    handle network messages.
]])

M['iproto.readahead'] = toline([[
    The size of the read-ahead buffer associated with a client connection.
    The larger the buffer, the more memory an active connection consumes,
    and the more requests can be read from the operating system buffer in
    a single system call. The recommendation is to make sure that the buffer
    can contain at least a few dozen requests. Therefore, if a typical tuple
    in a request is large, e.g. a few kilobytes or even megabytes, the
    read-ahead buffer size should be increased. If batched request processing
    is not used, it's prudent to leave this setting at its default.
]])

M['iproto.threads'] = toline([[
...
M['groups.*'] = 'A group name.'

M['groups.*.replicasets'] = 'Replica sets that belong to this group.'

M['groups.*.replicasets.*'] = 'A replica set name.'

M['groups.*.replicasets.*.instances'] = toline([[
    Instances that belong to this replica set.
]])

M['groups.*.replicasets.*.instances.*'] = 'An instance name.'

M['groups.*.replicasets.*.leader'] = toline([[
    A replica set leader. This option can be used to set a replica
    set leader when manual replication.failover is used.
]])

M['groups.*.replicasets.*.bootstrap_leader'] = toline([[
...
M['log.format'] = toline([[
    ...
    are supported: plain: a log entry is formatted as plain text, json: a log
    entry is formatted as JSON and includes additional fields.
]])

M['log.level'] = toline([[
    Specify the level of detail logs have. There are the following levels:
    0 - fatal, 1 - syserror, 2 - error, 3 - crit, 4 - warn, 5 - info,
    6 - verbose, 7 - debug. By setting log.level, you can enable logging of
    all events with severities above or equal to the given level.
]])

M['log.modules'] = toline([[
    ...
    Tarantool core messages. Specifically, it configures the logging level
    for messages logged from non-Lua code, including C modules.
]])

M['log.modules.*'] = toline([[
    Path to module. For example: you have module placed by the
    following path: test/module.lua. To configure logging levels, you need to
    provide module names corresponding to paths to these modules:
    ``` test.module: 'verbose' ```.
]])

M['log.nonblock'] = toline([[
    Specify the logging behavior if the system is not ready to write.
    If set to `true`, Tarantool does not block during logging if the system
    is non-writable and writes a message instead. Using this value may
    improve logging performance at the cost of losing some log messages.
...
M['log.syslog.server'] = toline([[
    Set a location of a syslog server. This option accepts one of the
    following values: An IPv4 address. Example: 127.0.0.1:514. A Unix socket
    path starting with `unix:`. Examples: `unix:/dev/log` on Linux or
    `unix:/var/run/syslog` on macOS. To write logs to syslog, you need to
    set `log.to` to `syslog`.
]])

M['log.to'] = toline([[
    Define a location Tarantool sends logs to. This option accepts the following
    values: `stderr`: write logs to the standard error stream,
...
M['memtx.allocator'] = toline([[
    Specify the allocator that manages memory for memtx tuples. Possible values:
    `system` - the memory is allocated as needed, checking that the quota is
    not exceeded. THe allocator is based on the malloc function. `small` - a
    slab allocator. The allocator repeatedly uses a memory block to allocate
    objects of the same type. Note that this allocator is prone to unresolvable
    fragmentation on specific workloads, so you can switch to system in such
    cases.
]])

M['memtx.max_tuple_size'] = toline([[
...
M['memtx.slab_alloc_granularity'] = toline([[
    ...
    following conditions: The value is a power of two. The value is greater
    than or equal to 4.
]])

M['memtx.sort_threads'] = toline([[
...
M['metrics'] = 'This section defines configuration parameters for metrics.'

M['metrics.exclude'] = toline([[
...
M['process'] = toline([[
    This section defines configuration parameters of the Tarantool
    process in the system.
]])

M['process.background'] = toline([[
...
M['process.coredump'] = toline([[
    Create coredump files. Usually, an administrator needs to call ulimit -c
    unlimited (or set corresponding options in systemd's unit file) before
    running a Tarantool process to get core dumps. If process.coredump is
    enabled, Tarantool sets the corresponding resource limit by itself and the
    administrator doesn't need to call ulimit -c unlimited (see man 3
    setrlimit). This option also sets the state of the dumpable attribute,
    which is enabled by default, but may be dropped in some circumstances
    (according to man 2 prctl, see `PR_SET_DUMPABLE`).
]])

M['process.pid_file'] = toline([[
    Store the process id in this file. This option may contain a relative file
    path. In this case, it is interpreted as relative to `process.work_dir`.
]])

M['process.strip_core'] = toline([[
...
M['process.work_dir'] = toline([[
    ...
    `process.work_dir` with chdir(2) after start. If set as a relative file
    path, it is relative to the current working directory, from where Tarantool
    is started. If not specified, defaults to the current working directory.
    Other directory and file parameters, if set as relative paths, are
    interpreted as relative to process.work_dir, for example, directories for
    storing snapshots and write-ahead logs.
]])
...
M['replication.bootstrap_strategy'] = toline([[
    Specifies a strategy used to bootstrap a replica set. The following
    strategies are available: `auto`: a node doesn't boot if half or more of
    the other nodes in a replica set are not connected. For example, if a
    replica set contains 2 or 3 nodes, a node requires 2 connected instances.
    In the case of 4 or 5 nodes, at least 3 connected instances are required.
    Moreover, a bootstrap leader fails to boot unless every connected node has
    chosen it as a bootstrap leader. `config`: use the specified node to
    bootstrap a replica set. To specify the bootstrap leader, use the
    `<replicaset_name>.bootstrap_leader` option. `supervised`: a bootstrap
    leader isn't chosen automatically but should be appointed
    `using box.ctl.make_bootstrap_leader()` on the desired node. `legacy`
    (deprecated since 2.11.0): a node requires the `replication_connect_quorum`
    number of other nodes to be connected. This option is added to keep the
    compatibility with the current versions of Cartridge and might be removed
    in the future.
]])

M['replication.connect_timeout'] = toline([[
    A timeout (in seconds) a replica waits when trying to connect to a master
    in a cluster. See orphan status for details. This parameter is different
    from replication.timeout, which a master uses to disconnect a replica when
    the master receives no acknowledgments of heartbeat messages.
]])

M['replication.election_fencing_mode'] = toline([[
    ...
    followers. In strict mode, a connection is considered dead if there are no
    responses for 2 * `replication.timeout` seconds on the current leader and
    4 * replication.timeout seconds on the followers. This improves the chances
    that there is only one leader at any time. Fencing applies to the instances
    that have the replication.election_mode set to candidate or manual. To turn
    off leader fencing, set `election_fencing_mode` to off.
]])

M['replication.election_mode'] = toline([[
    A role of a replica set node in the leader election process. The possible
    values are: `off`: a node doesn't participate in the election activities.
    `voter`: a node can participate in the election process but can't be a
    leader. `candidate`: a node should be able to become a leader. `manual`:
    allow to control which instance is the leader explicitly instead of relying
    on automated leader election.
    By default, the instance acts like a voter - it is read-only and may vote
    for other candidate instances. Once box.ctl.promote() is called, the
    instance becomes a candidate and starts a new election round. If the
    instance wins the elections, it becomes a leader but won't participate in
    any new elections.
]])

M['replication.election_timeout'] = toline([[
    Specifies the timeout (in seconds) between election rounds in the leader
    election process if the previous round ended up with a split vote. It is
    quite big, and for most of the cases, it can be lowered to 300-400 ms.
...
M['replication.failover'] = toline([[
    A failover mode used to take over a master role when the current master
    instance fails. The following modes are available: `off` - Leadership in a
    replica set is controlled using the database.mode option. In this case, you
    can set the database.mode option to rw on all instances in a replica set
    to make a master-master configuration. The default `database.mode` is
    determined as follows: `rw` if there is one instance in a replica set; `ro`
    if there are several instances. `manual` - Leadership in a replica set is
    controlled using the `<replicaset_name>.leader` option. In this case,
    a master-master configuration is forbidden. In the manual mode, the
    `database.mode` option cannot be set explicitly. The leader is configured
    in the read-write mode, all the other instances are `read-only.election` -
    Automated leader election is used to control leadership in a replica set.
    In the election mode, `database.mode` and `<replicaset_name>.leader`
    shouldn't be set explicitly. supervised (EE only) - Leadership in a
    replica set is controlled using an external failover coordinator. In the
    supervised mode, database.mode and <replicaset_name>.leader shouldn't be
    set explicitly.
]])

M['replication.peers'] = toline([[
    URIs of instances that constitute a replica set. These URIs are used by
    an instance to connect to another instance as a replica. Alternatively,
    you can use iproto.advertise.peer to specify a URI used to advertise the
    current instance to other cluster members.
]])

M['replication.peers.*'] = toline([[
    specifies URIs of replica set instances. For example:
    `- replicator:topsecret@127.0.0.1:3301`.
]])

M['replication.skip_conflict'] = toline([[
...
M['replication.sync_timeout'] = toline([[
...
M['replication.synchro_quorum'] = toline([[
    A number of replicas that should confirm the receipt of a synchronous
    transaction before it can finish its commit. This option supports
    dynamic evaluation of the quorum number. For example, the default
    value is N / 2 + 1 where N is the current number of replicas registered
    in a cluster. Once any replicas are added or removed, the expression
    is re-evaluated automatically. Note that the default value (at least
    50% of the cluster size + 1) guarantees data reliability. Using a
    value less than the canonical one might lead to unexpected results,
    including a split-brain. replication.synchro_quorum is not used on
    replicas. If the master fails, the pending synchronous transactions
    will be kept waiting on the replicas until a new master is elected.
]])

M['replication.synchro_timeout'] = toline([[
...
M['replication.timeout'] = toline([[
    ...
    request, a replica should return a heartbeat acknowledgment. If a master or
    replica gets no heartbeat message for 4 * replication.timeout seconds, a
    connection is dropped and a replica tries to reconnect to the master.
]])
...
M['security'] = toline([[
    (EE) This section defines configuration parameters related to various
    security settings.
]])

M['security.auth_delay'] = toline([[
    Specify a period of time (in seconds) that a specific user should wait
    for the next attempt after failed authentication. The
    security.auth_retries option lets a client try to authenticate the
    specified number of times before security.auth_delay is enforced. In
    the configuration below, Tarantool lets a client try to authenticate
...
M['security.auth_retries'] = toline([[
    ...
    attempt. The retry counter is reset after security.auth_delay seconds
    since the first failed attempt. For example, if a client tries to
    authenticate fewer than security.auth_retries times within
    security.auth_delay seconds, no authentication delay is enforced. The
    retry counter is also reset after any successful authentication
    attempt.
]])

M['security.auth_type'] = toline([[
    Specify a protocol used to authenticate users. The possible values
    are: `chap-sha1`: use the CHAP protocol with SHA-1 hashing applied to
    passwords. `pap-sha256`: use PAP authentication with the SHA256 hashing
    algorithm. Note that CHAP stores password hashes in the _user space
    unsalted. If an attacker gains access to the database, they may crack
    a password, for example, using a rainbow table. For PAP, a password is
    salted with a user-unique salt before saving it in the database, which
    keeps the database protected from cracking using a rainbow table.
]])

M['security.disable_guest'] = toline([[
...
M['security.password_lifetime_days'] = toline([[
...
M['security.password_min_length'] = toline([[
    Specify the minimum number of characters for a password.
]])

M['security.secure_erasing'] = toline([[
...
M['sharding.rebalancer_max_receiving'] = toline([[
...
M['sharding.rebalancer_max_sending'] = toline([[
    The degree of parallelism for parallel rebalancing.
]])

M['sharding.rebalancer_mode'] = toline([[
    Configure how a rebalancer is selected: `auto` (default): if there are no
    replica sets with the rebalancer sharding role (sharding.roles), a
    replica set with the rebalancer is selected automatically among all
    replica sets. `manual`: one of the replica sets should have the
    rebalancer sharding role. The rebalancer is in this replica set. off:
    rebalancing is turned off regardless of whether a replica set with the
    rebalancer sharding role exists or not.
]])

M['sharding.roles'] = toline([[
    Roles of a replica set in regard to sharding. A replica set can have the
    following roles: `router`: a replica set acts as a router. `storage`: a
    replica set acts as a storage. `rebalancer`: a replica set acts as a
    rebalancer. The rebalancer role is optional. If it is not specified, a
    rebalancer is selected automatically from the master instances of
    replica sets. There can be at most one replica set with the rebalancer
    role. Additionally, this replica set should have a storage role.
]])

M['sharding.roles.*'] = 'Role name.'

M['sharding.sched_move_quota'] = toline([[
    A scheduler's bucket move quota used by the rebalancer. `sched_move_quota`
    defines how many bucket moves can be done in a row if there are pending
    storage refs. Then, bucket moves are blocked and a router continues making
    map-reduce requests.
]])

M['sharding.sched_ref_quota'] = toline([[
    ...
    consistent map-reduce over the entire cluster. `sched_ref_quota` defines
    how many storage refs, therefore map-reduce requests, can be executed on
    the storage in a row if there are pending bucket moves. Then, storage refs
    are blocked and the rebalancer continues bucket moves.
]])

M['sharding.shard_index'] = toline([[
...
M['sharding.sync_timeout'] = toline([[
...
M['snapshot.snap_io_rate_limit'] = toline([[
...
M['vinyl.range_size'] = toline([[
    The default maximum range size for a vinyl index, in bytes. The maximum
    range size affects the decision of whether to split a range. If
    `vinyl.range_size` is specified (but the value is not null or 0), then it
    is used as the default value for the range_size option passed to
    `space_object:create_index()`. If vinyl.range_size is not specified (or
    is explicitly set to null or 0), and range_size is not specified when
    the index is created, then Tarantool sets a value later depending on
    performance considerations. To see the actual value, use
    `index_object:stat().range_size`.
]])

M['vinyl.read_threads'] = toline([[
    The maximum number of read threads that vinyl can use for concurrent
    operations, such as I/O and compression.
]])

M['vinyl.run_count_per_level'] = toline([[
...
M['wal.ext.spaces'] = toline([[
    ...
    options. The option is a key-value pair: The key is a space name
    (string). The value is a table that includes two optional boolean
    options: old and new. The format and the default value of these options
    are described in `wal.ext.old` and `wal.ext.new.`
]])
...
M['wal.retention_period'] = toline([[
    (EE) The delay in seconds used to prevent the Tarantool garbage collector
    from removing a write-ahead log file after it has been closed. If a node is
    restarted, wal.retention_period counts down from the last modification
    time of the write-ahead log file. The garbage collector doesn't track
    write-ahead logs that are to be relayed to anonymous replicas, such as:
    Anonymous replicas added as a part of a cluster configuration (see
    replication.anon). CDC (Change Data Capture) that retrieves data using
    anonymous replication. In case of a replica or CDC downtime, the
    required write-ahead logs can be removed. As a result, such a replica
    needs to be rebootstrapped. You can use wal.retention_period to prevent
    such issues. Note that wal.cleanup_delay option also sets the delay used
    to prevent the Tarantool garbage collector from removing write-ahead
    logs. The difference is that the garbage collector doesn't take into
    account wal.cleanup_delay if all the nodes in the replica set are up and
    running, which may lead to the removal of the required write-ahead logs.
