...
local function is_annotated(schema, name)
    if schema == nil or schema.computed == nil then
        return schema ~= nil and schema[name] == true
    end

    return schema.computed.annotations[name] == true
end
...
local function is_scalar(schema)
    return schema ~= nil and scalars[schema.type] ~= nil
end
...
--     discriminator = <function that returns a schema node from variants>,
--     <..annotations..>
-- })
local function union(union_def)
    assert(type(union_def.variants) == 'table')
    assert(next(union_def.variants) ~= nil)
    assert(union_def.discriminator == nil or
        type(union_def.discriminator) == 'function')
...
local function never_accept_nonempty_array(schema)
    ...
    --  | s:validate({'a', 'b', 'c'}) -- OK
    --
    -- If a map key is a union and at least one variant accepts numeric
    -- values, the map can accept a non-empty array too.
    --
    -- Example:
    --
...
local function can_have_simple_format(schema)
    if is_scalar(schema) then
        return schema.type ~= 'any'
    elseif schema.type == 'union' then
        for _, variant in ipairs(schema.variants) do
            if not can_have_simple_format(variant) then
                return false
            end
        end
        return true
...
local function scalar_discriminator_type(schema)
    assert(is_scalar(schema))

    if schema.type == 'integer' then
        -- Lua represents integers as numbers, so number and integer
        -- variants overlap for the default discriminator.
        return 'number'
    end
    return schema.type
end

local function union_default_discriminator_error(schema)
    local discriminator_types = {}
    for _, variant in ipairs(schema.variants) do
        if not is_scalar(variant) then
            return 'A union without a discriminator requires scalar ' ..
                'variants'
        end

        local discriminator_type = scalar_discriminator_type(variant)
        if discriminator_type == 'any' and #schema.variants > 1 then
            return 'A union without a discriminator cannot combine ' ..
                'the "any" scalar with other variants'
        end
        if discriminator_types[discriminator_type] then
            return 'A union without a discriminator requires variants ' ..
                'with distinct data types'
        end
        discriminator_types[discriminator_type] = true
    end

    return nil
end

local function normalize_union_variant(schema, variant, ctx)
    if variant == nil then
        return nil
    end
    for i, candidate in ipairs(schema.variants) do
        if variant == candidate then
            return i
        end
    end
    walkthrough_error(ctx, 'Union discriminator returned a schema node ' ..
        'that is not present in union variants')
end

local function discriminate_union_variant(schema, data, ctx)
    assert(schema.type == 'union')

    if schema.discriminator ~= nil then
        local w = {
        ...
        local variant = schema.discriminator(data, w)
        return normalize_union_variant(schema, variant, ctx)
    end

    for i, variant in ipairs(schema.variants) do
        assert(is_scalar(variant))
        if scalars[variant.type].validate_noexc(data) then
            return i
        end
    end
...
local function validate_impl(schema, data, ctx)
    ...
    elseif schema.type == 'union' then
        ...
        local errors = {}
        local pass = false
        local variant_index = discriminate_union_variant(schema, data, ctx)

        for i, variant in ipairs(schema.variants) do
            if variant_index == nil or i == variant_index then
                ...
                local ok, variant_err = pcall(validate_impl, variant, data,
                    subctx)
                pass = pass or ok
                if pass then
                    break
                ...
                errors[#errors + 1] = ('variant #%d: %s'):format(i,
                    variant_err)
            end
        end
        if not pass then
            if variant_index == nil then
                walkthrough_error(ctx, 'Data does not match any union ' ..
                    'variant (%s)', table.concat(errors, '; '))
            end
            walkthrough_error(ctx, 'Data does not match selected union ' ..
...
local function get_impl(schema, data, ctx)
    ...
    if #ctx.journey == 0 then
        if is_scalar(schema) and scalars[schema.type].normalize ~= nil and
                data ~= nil then
            local res, err = scalars[schema.type].normalize(data, schema)
            if err ~= nil then
                walkthrough_error(ctx, '%s', err)
            end
            return res
        end
        return data
    ...
    elseif schema.type == 'union' then
        local err = nil
        local variant_index = discriminate_union_variant(schema, data, ctx)
        local variants = variant_index == nil and schema.variants or
...
local function select_union_variant(schema, data, ctx)
    ...
    if variant_index == nil then
        local subctx = table.deepcopy(ctx)
        local ok, err = pcall(validate_impl, schema, data, subctx)
        assert(not ok)
        return nil, err
    end

    local variant = schema.variants[variant_index]
    local subctx = table.deepcopy(ctx)
    local ok, err = pcall(validate_impl, variant, data, subctx)
    if not ok then
        return nil, err
    end
...
local function normalize_f(data, w)
    if data == nil or not is_scalar(w.schema) then
        return data
    end
...
local function validate_schema_node_union(schema, ctx)
    if schema.type ~= 'union' then
        return
    end

    walkthrough_assert(ctx, type(schema.variants) == 'table',
        'Unexpected data type for union variants: %q',
        type(schema.variants))
    walkthrough_assert(ctx, table_is_array(schema.variants),
        'Union variants must be an array')
    ...
    if schema.discriminator == nil then
        local err = union_default_discriminator_error(schema)
        if err ~= nil then
            walkthrough_error(ctx, '%s', err)
        end
    end
...
local function map_from_env(env_var_name, raw_value, schema)
    local simple_format_allowed = schema.key.type == 'string' and
        can_have_simple_format(schema.value)

    local recommendation = simple_format_allowed and
    ...
    if not can_have_simple_format(schema.value) then
        error(('Use the JSON object format for environment variable %q: the ' ..
...
local function array_from_env(env_var_name, raw_value, schema)
    local simple_format_allowed = can_have_simple_format(schema.items)
    ...
    if not can_have_simple_format(schema.items) then
        error(('Use the JSON array format for environment variable %q: the ' ..
...
local function union_from_env(env_var_name, raw_value, schema)
    ...
    for i, variant in ipairs(schema.variants) do
        ...
        if ok then
            local ctx = {path = {}, name = env_var_name}
            local variant_index = discriminate_union_variant(schema, value, ctx)
            if variant_index == i then
                if pcall(validate_impl, variant, value, ctx) then
