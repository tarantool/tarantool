...
local function is_default_metric(name)
    return name == 'all' or
        require('metrics.tarantool').is_default_metric(name)
end
...
local function metric_is_enabled(name, include, exclude)
    local default_include, selector_include =
        split_metric_groups_and_selectors(include, 'all', 'none', 'all')
    local default_exclude, selector_exclude =
        split_metric_groups_and_selectors(exclude, {}, {}, {})

    if is_default_metric(name) then
        if metric_group_matches(name, default_exclude) then
            return false
        end
        return metric_group_matches(name, default_include)
    end

    if metric_selector_matches(name, selector_exclude) then
        return false
    end

    return metric_selector_matches(name, selector_include)
end
...
function methods.is_metric_enabled(self, name)
    ...
    local include = self:get('metrics.include')
    local exclude = self:get('metrics.exclude')
    return metric_is_enabled(name, include, exclude)
end
