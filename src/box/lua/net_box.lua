...
local function display_netbox_error(host, port, err, mode, reconnect_after)
    if mode == "warning" then
        log.warn("%s:%s: %s", host or "", port or "", err.message)
        if string.match(err.message, "Connection refused") then
            log.warn(string.format('will retry every %.2f second',
                                   reconnect_after))
        end
    else
        log.verbose("%s:%s: %s", host or "", port or "", err.message)
    end
end
...
local function new_sm(uri_or_fd, opts)
    ...
    local function callback(what, ...)
        ...
        if what == 'state_changed' then
            local state, err = ...
            remote.state = state
            remote.error = err and err.message or nil
            ...
            if state == 'error_reconnect' then
                ...
                local reconnect_after = remote.opts.reconnect_after
                local prev_err_code = err.prev and err.prev.code
                if err.code ~= last_err_code then
                    display_netbox_error(host, port, err, "warning",
                                         reconnect_after)
                else
                    if err.message ~= last_err_name and
                       prev_err_code ~= last_prev_err_code then
                        display_netbox_error(host, port, err, "warning",
                                             reconnect_after)
                    else
                        display_netbox_error(host, port, err, "verbose")
                    end
                end
