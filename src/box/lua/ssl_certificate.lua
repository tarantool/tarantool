...
local function new(opts)
    ...
    local function checker_loop()
        fiber.name(opts.fiber_name or 'ssl_certificate.checker')
        while true do
...
checker.register_discovery = function(name, discover)
    checks('string', 'function')
    for _, registered in ipairs(discoveries) do
        if registered.name == name then
            error(('SSL certificate discovery %q is already registered')
                  :format(name))
        end
    end
    table.insert(discoveries, {name = name, callback = discover})
end

-- Changes take effect on the next certificate check, including removal
-- of alerts for certificates that no remaining source discovers.
checker.unregister_discovery = function(name)
    checks('string')
    for i, registered in ipairs(discoveries) do
        if registered.name == name then
            table.remove(discoveries, i)
            return true
        end
    end
    return false
...
checker.register_discovery('iproto', function(config, records, add_certificate)
