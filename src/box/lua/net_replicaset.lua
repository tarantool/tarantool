...
local function check_options(tbl, tmpl, name)
    if type(tbl) ~= 'table' then
        error(name .. ' expected to be a map')
    end
    for k, v in pairs(tmpl) do
        if not string.find(v, type(tbl[k])) then
            local fmt = 'Option \'%s\' of %s is \'%s\' while must be \'%s\''
            error(string.format(fmt, k, name, type(tbl[k]), v))
        end
    end
    for k in pairs(tbl) do
        if tmpl[k] == nil then
            error('Unexpected option ' .. k .. ' in ' .. name, 2)
        end
    end
end
...
local function instance_status_change(replicaset, instance, status)
    ...
    for instance_name, instance_cfg in pairs(cfg.instances) do
        ...
        conn:watch('box.status', function(_key, value)
            ...
            if weak_ref.replicaset and weak_ref.instance then
                instance_status_change(weak_ref.replicaset, weak_ref.instance,
                                       status)
            end
        end)
...
local function connect_common(cfg_or_name, ...)
    assert(type(cfg_or_name) == 'string' or type(cfg_or_name) == 'table')
