...
struct xlog_reader_stream_msg {
	...
	struct error *error;
	/**
	 * Set if xlog EOF is reached. The `error` can be set as well
	 * in this case.
	 */
	bool eof;
};
...
xlog_reader_f(va_list ap)
{
	...
	bool opened;
	int rc = xlog_cursor_open(&raw_cursor, reader->filename);
	opened = rc == 0;
	if (opened)
	...
	while (true) {
		...
		if (rc == 0)
			rc = xlog_entry_cursor_next_batch(&cursor, &msg->batch);
		if (rc == -1) {
