 ...
 *
 * If the result is `XLOG_READER_DECODE_ERROR` or `XLOG_READER_ERROR` and
 * error is `XlogError` the reading can be continued. Otherwise continuing
 * reading is UB.
 *
 * Returns:
 *   XLOG_READER_OK - entry is set or set to NULL in case of EOF.
 *   XLOG_READER_ERROR - error reading entry.
 *   XLOG_READER_DECODE_ERROR - error decoding entry body, entry is set,
 *     but only header is valid in this case.
 * Diag is set if result is not `XLOG_READER_OK`.
 */
enum xlog_reader_result
