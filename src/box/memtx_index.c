			       ...
			       struct memtx_index_entry *successor)
{
	...
	*successor = memtx_index_entry_null;
	if (old_entry.tuple != NULL && old_entry.tuple == new_entry.tuple &&
...
/**
 * Finalize an entry inserted into an index.
 *
 * Functional indexes store the key tuple in the entry hint, so a successful
 * insert transfers one owned reference to the index.
 */
static void
memtx_index_inserted_entry_finalize(struct index *index,
				    struct memtx_index_entry entry)
{
	if (index->def->key_def->for_func_index && entry.tuple != NULL)
		tuple_ref((struct tuple *)entry.hint);
}
			  ...
			  struct memtx_index_entry *result)
{
	...
	if (memtx_index_replace_entry_impl(index, old_entry, new_entry, mode,
					   result, &unused) != 0)
		return -1;
	memtx_index_replaced_entry_cleanup(index, result);
	memtx_index_inserted_entry_finalize(index, new_entry);
	return 0;
}
	...
	struct index *index, struct memtx_index_replace_result_set *result)
{
	...
	while (memtx_index_replace_result_iterator_next(&it, &step)) {
		...
		memtx_index_replaced_entry_cleanup(index,
						   &step.replaced->entry);
		assert(step.inserted->entry.tuple == removed.tuple ||
		       (removed.tuple == NULL && is_mk_or_func));
	}
}

static struct memtx_index_replace_result *
memtx_index_replace_result_new(struct region *region, struct rlist *list)
{
	struct memtx_index_replace_result *result =
		xregion_alloc_object(region, typeof(*result));
...
memtx_index_replace_step_new(struct region *region,
			     struct memtx_index_replace_result_set *result)
{
	struct memtx_index_replace_step step;
	step.replaced =
		memtx_index_replace_result_new(region, &result->replaced);
	step.successor =
		memtx_index_replace_result_new(region, &result->successors);
	step.inserted =
		memtx_index_replace_result_new(region, &result->inserted);
	return step;
}
			     ...
			     struct memtx_index_replace_result_set *result)
{
	...
	if (new_tuple != NULL) {
		...
		for (size_t mk_idx = 0; mk_idx < mk_count; ++mk_idx) {
			...
			struct memtx_index_replace_step step =
				memtx_index_replace_step_new(region, result);
			    ...
			    struct memtx_index_replace_result_set *result)
{
	...
	struct memtx_index_replace_step step =
		memtx_index_replace_step_new(&fiber()->gc, result);
	step.inserted->entry = new_entry;
...
/** Dispatch tuple replacement by index type and collect its result set. */
static int
memtx_index_replace_dispatch(struct index *index, struct tuple *old_tuple,
			     struct tuple *new_tuple,
			     enum dup_replace_mode mode,
			     struct memtx_index_replace_result_set *result)
{
		    ...
		    struct tuple **result, struct tuple **successor)
{
	...
	*result = NULL;
	*successor = NULL;
	int rc = memtx_index_replace_dispatch(index, old_tuple, new_tuple,
					      mode, &replace_result);
	if (rc == 0) {
		*result =
