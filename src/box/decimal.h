...
/** See @ref box_decimal_storage_t. */
struct box_decimal_storage {
	/**
	 * Decimal data.
	 *
 ...
 * Usage example:
 *
 *  | box_decimal_storage_t storage;
 *  | box_decimal_t *dec = (box_decimal_t *)&storage;
 *  | box_decimal_<...>(dec);
 *
 * The only legal usage is to allocate a memory for the structure
 * and cast its address to (box_decimal_t *) to use with
 * box_decimal_*() functions.
 */
typedef struct box_decimal_storage box_decimal_storage_t;

/** See @ref box_decimal_t. */
struct box_decimal;

/**
 * Decimal (opacue structure).
 *
 * It allow to represent a wide range of decimal values without
 * presision loss and provides a set of mathematical operations on
 * them.
 *
 * The range is not specified, but it holds at least 38
 * significant decimal digits and can represent numbers with a
 * fractional part.
 *
 * The data format is unspecified and can be changed. Modules and
 * applications that aims to be compatible with future tarantool
 * versions must not lean on particular data representation within
 * the structure.
 */
...
/**
 * Amount of bytes enough to hold any decimal value.
 *
 * This value is less or equal to sizeof(box_decimal_storage_t).
 * It is valid only for particular tarantool version. The value is
 * a runtime constant: it is not changed during tarantool
 * execution.
 *
 * The function is suitable for dynamic allocation of a storage
 * for a decimal type, especially when the memory is managed by
 * a runtime of another language.
 *
 * @return max decimal size in bytes on current tarantool version
 */
API_EXPORT size_t
box_decimal_storage_size(void);

/**
 * Alignment of the decimal storage.
 *
 * It is less or equal to alignof(box_decimal_storage_t). The
 * value is valid only for particular tarantool version. The value
 * is a runtime constant: it is not changed during tarantool
 * execution.
 *
 * @return decimal type alignment on current tarantool version
 */
API_EXPORT size_t
box_decimal_storage_align(void);

/**
 * Amount of bytes to hold any box_decimal_to_string() result.
 *
 * Similarly to box_decimal_storage_size() it is a runtime
 * constant and valid only for particular tarantool version. It is
 * less or equal to @ref BOX_DECIMAL_STRING_BUFFER_SIZE.
 *
 * @return max decimal string size in bytes on current tarantool
 *         version
 */
API_EXPORT size_t
 ...
 *
 * The buffer must have at least box_decimal_string_buffer_size()
 * or @ref BOX_DECIMAL_STRING_BUFFER_SIZE bytes.
 *
 ...
 */
static_assert(sizeof(box_decimal_storage_t) == 64,
	      "sizeof(box_decimal_storage_t) == 64");
static_assert(alignof(box_decimal_storage_t) == 8,
	      "alignof(box_decimal_storage_t) == 8");
