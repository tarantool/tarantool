...
local function default_context(level)
    ...
    local ctx = require('log')._internal.default_context()
...
local function say(self, level, fmt, ...)
    ...
    elseif type_fmt == 'table' then
        if ffi.C.log_format == ffi.C.SF_JSON then
            if self and self.context_generator ~= nil then
                local context_generator_name = self.context_generator
                if type(rawget(_G, context_generator_name)) == 'function' then
                    msg = _G[context_generator_name](level)
                elseif box.schema.func.exists(context_generator_name) then
                    msg = box.func[self.context_generator]:call({level})
                end
            end

            if msg == nil then
                msg = default_context(level)
            ...
            -- @TODO do we check if k doesn't exist here?
            ...
            if self and self.fields_order then
...
local function log_configure(self, cfg, box_api)
    ...
    log_debug("log.cfg({log=%s, level=%s, nonblock=%s, format=%s, " ..
              ...
              cfg.context_generator, cfg.fields_order)
end
...
log_main.default_context = default_context
