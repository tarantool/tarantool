...
local MAX_UINT64 = 0xfffffffffffff800
local MIN_SINT32 = -2^31
local MAX_SINT32 = 2^31 - 1
local MIN_SINT64 = -0x8000000000000000
local MAX_SINT64 = 0x7ffffffffffffc00
...
local function validate_range(field_def, value, range)
    ...
    -- To avoid incorrect comparison unsigned cdata should not be
    -- checked for lower limit. If one of the limits is 'nil' this
    -- function skips the comparison with this limit.
...
encode_field = function(protocol, field_def, value, ignore_repeated)
    ...
    elseif is_message(protocol, field_def) then
        ...
        return wireformat.encode_len(field_def.id, encoded_msg)
    else
        assert(false)
    end
end
