...
* A Branch has N **Regions** - a vinyl Region is like a B-tree `page`.
...
Branches reaches this maximum, the vinyl scheduler wakes a **Compaction Thread**
...
The number of branches became too big, so the vinyl scheduler starts the
...
* The best vinyl performance will occur if there is no setting of a memory limit,
  ...
  vinyl scheduler will give priority to the Nodes that have the largest
...
During the Get (search), vinyl first finds the correct Node by looking in the
